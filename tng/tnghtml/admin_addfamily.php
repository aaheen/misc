...
if( $husband ) {
	...
	$husborder = !empty($husbrow['husborder']) ? ($husbrow['husborder'] + 1) : 0;
}
else {
	$husbliving = 0;
	$husborder = 0;
...
if( $wife ) {
	...
	$wifeorder = !empty($wiferow['wifeorder']) ? ($wiferow['wifeorder'] + 1) : 0;
}
else {
	$wifeliving = 0;
	$wifeorder = 0;
}
$familyliving = (!empty($living) || $husbliving || $wifeliving) ? 1 : 0;
