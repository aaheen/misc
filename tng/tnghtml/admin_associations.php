...
<p>
<?php if( $allow_add ) { ?>
	<input type="button" class="btn" value="  <?php echo $admtext['addnew']; ?>  " onclick="document.newassocform1.reset();assocType='I';gotoSection('associations','addassociation');" />&nbsp;
<?php } ?>
...
<?php
	if( $assocresult && $assoccount ) {
		while ( $assoc = tng_fetch_assoc( $assocresult ) ) {
			...
			$actionstr = $allow_edit ? "<a href=\"#\" onclick=\"return editAssociation({$assoc['assocID']}, '{$tree}');\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>" : "";
			$actionstr .= $allow_delete ? "<a href=\"#\" onclick=\"return deleteAssociation({$assoc['assocID']},'$personID','{$assoc['assocID']}','$tree');\" title=\"{$admtext['text_delete']}\" class=\"newsmallericon\"><img src=\"img/times.png\" /></a>" : "";
...
<table width="95%" border="0" cellpadding="2" class="normal">
	...
	<tr><td><span id="person_label"><?php echo $admtext['personid']; ?></span><span id="family_label" style="display:none"><?php echo $admtext['familyid']; ?></span>:</td>
		...
		<a href="#" onclick="return findItem(assocType,'passocID',null,'<?php echo $tree; ?>','<?php echo $assignedbranch; ?>');" title="<?php echo $admtext['find']; ?>"><img src="img/search.png" alt="<?php echo $admtext['find']; ?>" class="alignmiddle" /></a></td>
	</tr>
