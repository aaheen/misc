...
$defsdone = 0;
if(!isset($noeventmedia)) $noeventmedia = 0;

$noeventmediastr = $noeventmedia == "1" ? " AND eventID != \"\"" : "";
while( $distinctplink = tng_fetch_assoc($result) ) {
	...
	$query2 = "SELECT medialinkID FROM ($medialinks_table, $media_table) 
		WHERE $medialinks_table.mediaID = $media_table.mediaID AND $medialinks_table.gedcom=\"$tree\" AND personID = \"{$distinctplink['personID']}\" AND thumbpath != \"\"$noeventmediastr 
		ORDER BY ordernum";
