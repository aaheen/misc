...
// $tree is not always set when entering with a new search.
if( !isset($tree) ) $tree = "";
if( !empty($newsearch) ) {
	$exptime = 0;
...
if( $test_type )
	$wherestr .= $wherestr ? " AND test_type = \"" . cleanIt($test_type) . "\"" : "test_type = \"" . cleanIt($test_type) . "\"";
if( $test_group )
	$wherestr .= $wherestr ? " AND dna_group = \"" . cleanIt($test_group) . "\"" : "dna_group = \"" . cleanIt($test_group) . "\"";
...
<div class="admin-main">
	...
	<input type="text" name="searchstring" class="longfield" value="<?php echo $originalstring; ?>">
	<input type="submit" name="submit2" class="btn" value="<?php echo $admtext['search']; ?>" class="aligntop">
	<input type="submit" name="reset" class="btn" value="<?php echo $admtext['reset']; ?>" class="aligntop" onclick="resetForm();">
...
<?php
		if( $allow_delete ) {
?>
				<input type="submit" name="xdnaaction" class="btn bigfield" value="<?php echo $admtext['deleteselected']; ?>" onClick="return confirm('<?php echo addslashes($admtext['confdeleterecs']); ?>');">
<?php
		}
?>
			</div>
...
<?php
	if( $numrows ) {
		$actionstr = "";
		if( $allow_edit )
			$actionstr .= "<a href=\"admin_edit_dna_test.php?testID=xxx\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
		$actionstr .= "<a href=\"" . $show_dna_test_url . "testID=xxx\" target=\"_blank\" title=\"{$admtext['test']}\" class=\"newsmallericon\"><img src=\"img/play-circle.png\" /></a>";
		if( $allow_delete )
