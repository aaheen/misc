...
$query3 = "SELECT alinkID, entityID, eventID, people.lastname as lastname, people.lnprefix as lnprefix, people.firstname as firstname, people.prefix as prefix, people.suffix as suffix, people.nameorder as nameorder, people.title as title, ate.gedcom, treename,
	people.birthdate, people.birthdatetr, people.altbirthdate, people.altbirthdatetr, people.deathdate, people.deathdatetr, people.living, people.private, people.branch,
	...
	UNION
	SELECT alinkID, entityID, eventID, null, null, null, null, null, null, null, gedcom, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, linktype
...
<td class="tngshadow databack">
	<div><div id="thumbholder" style="float:left;<?php if(!$photo) echo "display:none"; ?>"><?php echo $photo; ?></div><span class="nameheader"><?php echo $row['albumname'] . ": </span><br/>" . $row['description']; ?></div>
	<?php
...
<td class="tngshadow databack">
	...
	<p style="padding-top:12px">
		<input type="button" class="btn" value="<?php echo $admtext['addmedia']; ?>" onclick="return openAlbumMediaFind();"> <?php echo $admtext['selmedia'] . " (<a href=\"admin_newmedia.php\" target=\"_blank\">" . $admtext['uploadfirst'] . "</a>)"; ?>
	</p>

	<p>&nbsp;<strong><?php echo $admtext['inclmedia']; ?>:</strong> <?php echo $admtext['emoptions']; ?></p>
	<table id="ordertbl" width="100%" cellpadding="3" cellspacing="1" border="0" class="fieldname">
		<tr>
			<td class="fieldnameback" style="width:106px">&nbsp;<b><?php echo $admtext['text_sort']; ?></b>&nbsp;</td>
			...
			<td class="fieldnameback" style="width:150px">&nbsp;<b><?php echo $admtext['date']; ?></b>&nbsp;</td>
			<td class="fieldnameback" style="width:101px">&nbsp;<b><?php echo $admtext['mediatype']; ?></b>&nbsp;</td>
		</tr>
	</table>
...
<?php
	...
	while( $lrow = tng_fetch_assoc( $result2 ) )
	{
		...
		echo "<td class=\"lightback\" style=\"width:" . ($thumbmaxw+10) . "px;text-align:center;\">";
		...
		echo "<td class=\"lightback\"><a href=\"admin_editmedia.php?mediaID={$lrow['mediaID']}\">{$lrow['description']}</a><br/>" . strip_tags($truncated) . "<br/>";
		...
		echo "</div></td>\n";
		echo "<td class=\"lightback\" style=\"width:150px;\" valign=\"top\">{$lrow['datetaken']}&nbsp;</td>\n";
		echo "<td class=\"lightback\" style=\"width:100px;\" valign=\"top\">" . $text[$lmediatypeID] . "&nbsp;</td>\n";
		echo "</tr></table>";
		echo "</div>\n";
		$count++;
	}
...
<td class="tngshadow databack">
	...
	<table cellspacing="2" style="padding-top:12px">
		<tr>
			...
			<td colspan="2"><?php echo $admtext['id']; ?></td>
		</tr>
		<tr>
			<td>
...
?>
			...
			<td>
				<select name="linktype1" class="bigselect">
			...
			<td><input type="button" class="btn" value="<?php echo $admtext['add']; ?>" onclick="return addMedia2EntityLink(document.form1);"> &nbsp;<?php echo $admtext['text_or']; ?>&nbsp;</td>
			<td><a href="#" onclick="return findItem(findform.linktype1.options[findform.linktype1.selectedIndex].value,'newlink1',null,findform.tree1.options[findform.tree1.selectedIndex].value,'<?php echo $assignedbranch; ?>','a_<?php echo $albumID; ?>');" title="<?php echo $admtext['find']; ?>">
	...
	<p>&nbsp;<strong><?php echo $admtext['existlinks']; ?>:</strong> <?php echo $admtext['eloptions']; ?></p>
	<table cellpadding="3" cellspacing="1" class="rounded-table">
	<tbody id="linktable">
		<tr>
	...
	if( $result3 ) {
		while( $plink = tng_fetch_assoc( $result3 ) )
		{
			...
			if( $plink['personID'] != NULL ) {
				$type = "person";
				$id = " (" . $plink['personID'] . ")";
				$prights = determineLivingPrivateRights($plink);
				$plink['allow_living'] = $prights['living'];
				$plink['allow_private'] = $prights['private'];
				$name = getName( $plink );
			}
			...
			if($type != "place")
				echo "<a href=\"#\" title=\"{$admtext['edit']}\" onclick=\"return editMedia2EntityLink({$plink['alinkID']});\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
			echo "<a href=\"#\" title=\"{$admtext['removelink']}\" onclick=\"return deleteMedia2EntityLink({$plink['alinkID']});\" title=\"{$admtext['removelink']}\" class=\"newsmallericon\"><img src=\"img/times.png\" /></a>\n";
			echo "</td>\n";
			echo "<td class=\"lightback\">" . $admtext[$type] . "</td>\n";
			echo "<td class=\"lightback\">$name$id&nbsp;</td>\n";
			echo "<td class=\"lightback\">{$plink['treename']}</td>\n";
			echo "<td class=\"lightback\" id=\"event_{$plink['alinkID']}\">$eventstr&nbsp;</td>\n";
			echo "</tr>\n";
		}
