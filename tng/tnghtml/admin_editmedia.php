...
$row['description'] = !empty($row['description']) ? preg_replace("/\"/", "&#34;",$row['description']) : "";
$row['notes'] = preg_replace("/\"/", "&#34;",$row['notes']);
$row['datetaken'] = !empty($row['datetaken']) ? preg_replace("/\"/", "&#34;",$row['datetaken']) : "";
$row['placetaken'] = !empty($row['placetaken']) ? preg_replace("/\"/", "&#34;",$row['placetaken']) : "";
$row['owner'] = !empty($row['owner']) ? preg_replace("/\"/", "&#34;",$row['owner']) : "";
if(!empty($row['map'])) {
...
if($row['abspath']) 
	$row['path'] = !empty($row['path']) ? preg_replace("/&/", "&amp;",$row['path']) : "";
...
if( $row['thumbpath'] && file_exists("$rootpath$usefolder/$treestr" . $row['thumbpath'])) {
	...
	$photo = "<img class=\"adminthumb\" src=\"$usefolder/$treestr" . str_replace("%2F","/",rawurlencode( $row['thumbpath'] )) . "\" width=\"$photowtouse\" height=\"$photohtouse\" style=\"border-color:#000000;margin-right:6px\"></span>\n";
}
else
	$photo = "";
...
if($row['path'] && ($form == "JPG" || $form == "JPEG" || $form == "GIF" || $form == "PNG" ) ) {
	$size = @GetImageSize( "$rootpath$usefolder/$treestr" . $row['path'] );
	$isphoto = TRUE;
}
else {
	$size = "";
...
if( $map['key'] && $isConnected) {
    ...
    echo "<script type=\"text/javascript\" src=\"{$http}://maps.googleapis.com/maps/api/js?language={$text['glang']}$loadingasync$mapkeystr\"></script>\n";
}
?>
<?php
	$onload = $onunload = "";
...
<td class="tngshadow databack">
	<table cellpadding="0" cellspacing="0" class="label">
		<tr>
			...
			<td>
				<span class="nameheader"><?php echo $row['description']; ?></span><br/>
				<?php echo $row['notes']; ?>
...
?>
	...
	<tr><td valign="top" colspan="2"><input type="checkbox" name="abspath" value="1"<?php if( $row['abspath'] ) echo " checked"; ?> onClick="toggleMediaURL();"> <?php echo $admtext['abspath']; ?></td></tr>
	...
	<tr id="pathrow">
		...
		<td><input type="text" value="<?php if( !$row['abspath'] ) echo $row['path']; ?>" name="path" id="path" size="60" onchange="prepopulateThumb();"><input type="hidden" value="<?php if( !$row['abspath'] ) echo $row['path']; ?>" id="path_org" name="path_org"><input type="hidden" id="path_last" name="path_last"> <input type="button" class="btn" value="<?php echo "{$admtext['select']}..."; ?>" name="photoselect" onclick="javascript:var folder = document.form1.usecollfolder[0].checked ? document.form1.mediatypeID.options[document.form1.mediatypeID.selectedIndex].value : 'media';FilePicker('path',folder);"></td>
	</tr>
...
<!-- history section -->
	<tr id="bodytextrow"><td valign="top"><?php echo $admtext['bodytext']; ?>:</td><td><textarea wrap cols="90" rows="11" name="bodytext" id="bodytext"><?php echo $row['bodytext']; ?></textarea></td></tr>
...
?>
	<tr>
		<td colspan="2"><strong><br/><?php echo $admtext['thumbnailfile']; ?></strong><br />
		<span class="red"><?php echo $admtext['imglib']; ?></span></td>
	</tr>
<?php
	}
?>
	...
	<tr>
		<td><?php echo $admtext['pathwithinphotos']; ?>**:</td>
		<td><input type="text" value="<?php echo $row['thumbpath']; ?>" name="thumbpath" id="thumbpath" size="60"><input type="hidden" value="<?php if( !$row['abspath'] ) echo $row['thumbpath']; ?>" id="thumbpath_org" name="thumbpath_org"><input type="hidden" id="thumbpath_last" name="thumbpath_last"> <input type="button" class="btn" value="<?php echo "{$admtext['select']}..."; ?>" name="thumbselect" OnClick="javascript:var folder = document.form1.usecollfolder[0].checked ? document.form1.mediatypeID.options[document.form1.mediatypeID.selectedIndex].value : 'media';FilePicker('thumbpath',folder);"></td>
	</tr>
	<tr>
	...
	<tr id="vidrow1">
		<td colspan="2">
			<br/>
			<?php echo $admtext['width']; ?>: <input type="text" name="width" value="<?php echo $row['width']; ?>" size="5"> &nbsp;
			<?php echo $admtext['height']; ?>: <input type="text" name="height" value="<?php echo $row['height']; ?>" size="5"> &nbsp;
			<p><?php echo "({$admtext['controller']})"; ?></p>
		</td>
	</tr>
	</table>
	<p class="smaller">
...
<td class="tngshadow databack">
	...
	<br/>
	<table class="label">
	<tr><td valign="top"><?php echo $admtext['title2']; ?>:</td><td><textarea wrap cols="70" rows="3" name="description"><?php echo $row['description']; ?></textarea></td></tr>
...
<?php
	if( $assignedtree ) {
		...
		echo "<select name=\"tree\" class=\"bigselect\" onchange=\"$('#microtree').val($(this).val());\">";
...
<!-- headstone section -->
<?php
	if(!empty($row['cemeteryID'])) {
		$query = "SELECT cemname, cemeteryID, city, county, state, country FROM $cemeteries_table WHERE cemeteryID = \"{$row['cemeteryID']}\"";
		$cemresult = tng_query($query);
		$cemrow = tng_fetch_assoc($cemresult);
		$cemname = $cemrow['cemname'];
		if(!empty($cemrow['city']))
			$cemname .= " - {$cemrow['city']}";
		if(!empty($cemrow['county']))
			$cemname .= ", {$cemrow['county']}";
		if(!empty($cemrow['state']))
			$cemname .= ", {$cemrow['state']}";
		if(!empty($cemrow['country']))
			$cemname .= ", {$cemrow['country']}";
		$cemname .= " &nbsp; "; 
...
?>
	<tr id="cemrow">
		...
		<td>
			<input type="hidden" id="cemeteryID" name="cemeteryID" value="<?php echo $row['cemeteryID']; ?>" />
			<span id="cemname"><?php echo $cemname; ?></span>
			<input type="button" class="btn" value="<?php echo $admtext['select']; ?>" onclick="return openFindCemeteryForm('cemname');" />
			</div>
		</td>
	</tr>
	<tr id="hsplotrow"><td><?php echo $admtext['plot']; ?>:</td><td><textarea wrap cols="70" rows="2" name="plot"><?php echo $row['plot']; ?></textarea></td></tr>
	<tr id="hsstatrow">
		...
		<td>
			<select name="status" class="bigselect">
				...
				<option value="moved"<?php if( $row['status'] == 'moved' ) echo " selected"; ?>><?php echo $admtext['moved']; ?></option>
				<option value="cremated"<?php if( $row['status'] == 'cremated' ) echo " selected"; ?>><?php echo $admtext['cremated']; ?></option>
				<option value="atsea"<?php if( $row['status'] == "atsea" ) echo " selected"; ?>><?php echo $admtext['atsea']; ?></option>
			</select>
		</td>
	</tr>

	<tr><td colspan="2" />&nbsp;</tr>
	<tr><td valign="top" colspan="2"><input type="checkbox" name="alwayson" value="1" id="img_alwayson"<?php if( $row['alwayson'] ) echo " checked"; ?> onchange="if($(this).is(':checked')){$('#img_private').prop('checked',false);}"> <?php echo $admtext['alwayson']; ?></td></tr>
	...
	<tr><td valign="top" colspan="2"><input type="checkbox" name="private" value="1" id="img_private"<?php if( $row['private'] ) echo " checked"; ?> onchange="if($(this).is(':checked')){$('#img_alwayson').prop('checked',false);}"> <?php echo $admtext['private']; ?></td></tr>
...
<tr>
<td class="tngshadow databack">
	<?php echo displayToggle("plus3",0,"imagemapdiv",$admtext['imgtags'],$admtext['mapinstr2']); ?>

	<div id="imagemapdiv" class="normal" style="display:none">
	<br />
	<p style="max-width:700px"><?php echo $admtext['mapinstr3']; ?></p>

	<table border="0" cellspacing="0" cellpadding="2" class="label">
		<tr>
			<td><?php echo $admtext['tree']; ?>:</td>
			<td>
				<select name="maptree" class="bigselect" id="maptree">
			<?php
				$lookfortree = !empty($row['gedcom']) ? $row['gedcom'] : $usetree;
				for( $j = 1; $j <= $treenum; $j++ ) {
					echo "	<option value=\"$trees[$j]\"";
					if($trees[$j] == $lookfortree) echo " selected";
			...
			<td>
				<select name="linktype" class="bigselect" id="linktype">
			...
			<td><?php echo $admtext['title'] . ": "; ?><input type="text" name="urltitle" id="urltitle"/></td>
			<td><?php echo $admtext['url'] . ": "; ?><input type="text" name="urladdress" id="urladdress" class="longfield" /></td>
		</tr>
	</table><br/>
	<?php
...
<tr>
<td class="tngshadow databack">
	<?php echo displayToggle("plus4",$placeopen,"placeinfo",$admtext['placetaken'],""); ?>

	<div id="placeinfo"<?php if(!$placeopen) echo " style=\"display:none\""; ?>>
	<table class="topbuffer label" width="100%">
	<tr>
		<td width="150"><?php echo $admtext['placetaken']; ?>:</td>
		<td><input type="text" name="place" id="place" value="<?php echo $row['placetaken']; ?>" size="40" style="float:left"> &nbsp;&nbsp;		<a href="#" onclick="return openFindPlaceForm('place');" title="<?php echo $admtext['find']; ?>" class="newsmallicon dn3px"><img src="img/search.png" alt="<?php echo $admtext['find']; ?>" class="alignmiddle" /></a></td>
	</tr>
<?php
	if( $map['key'] ) {
?>
...
?>
	<input type="hidden" value="" name="cw" />
...
function validateForm( ) {
	...
	if(!frm.notes.value.length && !frm.description.value) {
		alert("<?php echo $admtext['enterphotodesc']; ?>");
		rval = false;
	}
	else if( frm.path.value.length == 0 && frm.mediaurl.value.length == 0 && frm.bodytext.value.length == 0 && frm.mediatypeID.options[frm.mediatypeID.selectedIndex].value != "headstones" ) {
...
function toggleAll(display) {
	...
	if(jQuery('#imagemapdiv').length)
		toggleSection('imagemapdiv','plus3',display);
	toggleSection('placeinfo','plus4',display);
	return false;
}
</script>
...
function saveRectangle(tree, linktype, id, label) {
	...
	jQuery.ajax({
		...
		success: function(vars){
			...
			var overhang = 100 - $("#mlboxtmp").width();
			var stylestr = "";
			if(overhang > 0) {
				side = -1 * Math.round(overhang/2);
				stylestr = "left:" + side + "px;right:" + side + "px;"; 
			}
			else
				stylestr = "";
			$("#mlboxtmp").append('<div class="imagetag" style="top:' + (current.height() + 1) + 'px;' + stylestr + '">'+vars.name+'</div>')
...
function wireUpBox(rbox) {
	rbox.mousedown(function(e) {
        ...
        target.removeClass('bunselected').addClass('bselected').css('z-index',2);
		bx = e.pageX;
		by = e.pageY;
		boxmoving = true;
		poschanged = false;
...
$(document).ready(function() {
	...
	$('#myimg').mouseup(function(e) {
		if(x1 != e.pageX || y1 != e.pageY) {
		    ...
		    if(jQuery('#urltitle').val()) {
		    	saveRectangle('', 'U', jQuery('#urladdress').val(), jQuery('#urltitle').val());
		    	jQuery('#urltitle').val('');
		    }
		   	else
				findItem(jQuery('#linktype').val(),'imagemap','',tree, assignedbranch);
		}
		else
			box.remove();
		//$("#current").attr({ id: '' });
