...
if(is_numeric($ID))
	...
	$wherestr = "place = \"$ID\"";
	if(!empty($tree) && !$tngconfig['places1tree'])
...
<script> <?php include("niceditmsgs.php"); ?> </script>
<script type="text/javascript" src="js/nicedit.js"></script>
<script type="text/javascript">
//<![CDATA[
bkLib.onDomLoaded(function() {
    new nicEditor({fullPanel: true, maxHeight: 300}).panelInstance('mynotes');
});

function addNewMedia() {
	alert("<?php echo $admtext['redirect']; ?>");
	window.open('admin_newmedia.php?place=<?php echo addslashes($orgplace); ?>&tree=<?php echo $tree; ?>&linktype=L','_blank');
	return false;
}
//]]>
</script>
...
function addCemLink(cemeteryID) {
	...
	jQuery.ajax({
		...
		success: function(vars){
			...
			var actionstr = '<a href="#" onclick="return deleteCemLink(\'' + cemeteryID + '\');" title="' + delmsg + '"  style="height:17px" class="oldicon"></a>';
			...
			insertCell(newtr,1,"nw aligntop",vars.location);
			tnglitbox.remove();
...
<div class="admin-main">
	...
	<p class="nameheader"><?php echo $row['place']; ?></p>
...
?>
	...
	<tr><td><?php echo $admtext['place']; ?>:</td><td><input type="text" value="<?php echo $row['place']; ?>" name="place" id="place" size="70"></td></tr>
<?php
...
?>
	...
	<tr>
		...
		<td>
			<select name="placelevel" class="bigselect">
				<option value=""></option>
...
?>
	...
	<tr>
		...
		<td>
			<table id="cemeteries" cellpadding="3" cellspacing="1" border="0">
				<tbody id="cemeteriestblbody">
<?php
	//get cemeteries with no place assoc
	$query = "SELECT cemeteryID, cemname, city, county, state, country FROM $cemeteries_table WHERE place = \"" . addslashes($orgplace) . "\" ORDER BY cemname";
	...
	while( $cemrow = tng_fetch_assoc($cemresult) ) {
		...
		$actionstr = $allow_delete ? "<a href=\"#\" onclick=\"return deleteCemLink('{$cemrow['cemeteryID']}');\" title=\"{$admtext['text_delete']}\"><img src=\"img/times.png\" style=\"height:17px\" class=\"oldicon\"/></a>" : "&nbsp;";
		if($allow_edit)
		...
		echo "<tr id=\"row_{$cemrow['cemeteryID']}\"><td class=\"nw\">$actionstr</td><td class=\"nw aligntop\">$location</td></tr>\n";
	}
?>
				</tbody>
			</table>
			<input type="button" class="btn" value="<?php echo $admtext['linkcem']; ?>" onclick="pickCemetery();" />
			<img src="img/earth.gif" alt="<?php echo $admtext['geocopy']; ?>" width="15" height="15" class="oldicon"/> = <?php echo $admtext['geocopy']; ?>
		</td>
	</tr>
	<tr><td valign="top"><?php echo $admtext['notes']; ?>:</td><td><textarea wrap cols="90" rows="5" name="notes" id="mynotes"><?php echo $row['notes']; ?></textarea></td></tr>
<?php
	if( !$assignedbranch ) {
?>
...
<?php
		$close_command = "window.location.href='admin_places.php';";
	}
?>
