...
function validateForm( ) {
	...
	else if( document.form1.password.value.length < <?php echo $pwd_min_length; ?> || document.form1.password.value.length > 128 ) {
		alert("<?php echo $pwdlenstr; ?>");
		document.form1.password.focus();
		rval = false;
	}
	else if( document.form1.email.value.length == 0 || !checkEmail(document.form1.email.value)) {
		alert(enteremail);
	...
	<table class="label">
		...
		<tr><td><?php echo $admtext['username']; ?>:</td>
			<td>
				...
				<input type="button" class="btn" value="<?php echo $admtext['check']; ?>" onclick="checkNewUser(document.form1.username,document.form1.orguser);">
...
?>
		<tr><td valign="top"><?php echo $admtext['notes']; ?>:</td><td><textarea cols="50" rows="4" name="notes"><?php echo stripcslashes($row['notes']); ?></textarea></td></tr>
		<tr>
			<td>
				<?php echo $admtext['tree']; ?> / <?php echo $admtext['personid']; ?>:
			</td>
			<td>
				<select name="mynewgedcom" class="bigselect">
				<option value=""></option>
<?php
			...
			<tr>
				...
				<td valign="top">
					...
					<p>
						...
						<input type="checkbox" name="form_allow_private_notes" value="1"<?php if( $row['allow_private_notes'] > 0 ) echo " checked=\"checked\""; ?>> <?php echo $admtext['allow_private_notes']; ?><br/>
...
?>
	...
	<div id="restrictions" <?php if($adminaccess) echo "style='display:none;'"; ?>>
        <table class="label">
		<tr><td>
		<?php echo $admtext['tree']; ?>*:</td><td>
			<select name="gedcom" class="bigselect" id="treeselect" onChange="var tree=getTree(this); if( !tree ) tree = 'none'; <?php echo $swapbranches; ?>">
				<option value=""></option>
<?php
	$treeresult = tng_query($treequery);
...
?>
		...
		<tr><td><?php echo $admtext['branch']; ?>**:</td><td>
<?php
...
?>
	...
	<input type="hidden" name="newuser" value="<?php echo isset($newuser) ? $newuser : ""; ?>">
	...
	<input type="submit" name="submit" accesskey="s" class="btn savestay" value="<?php echo $admtext['savereturn']; ?>">
