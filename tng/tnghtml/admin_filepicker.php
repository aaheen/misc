...
$img = "";
if( $path == "gedcom" ) {
	$treestr = !empty($tngimpcfg['gedtrees']) && $assignedtree ? "/$assignedtree" : "";
	$tngpath = !empty($tngimpcfg['saveconfig']) ? $subroot . $gedpath : $gedpath . $treestr;
	$fulltngpath = !empty($tngimpcfg['saveconfig']) ? $subroot . $gedpath : $rootpath . $gedpath . $treestr;
	$displaypath = $treestr ? $treestr : $tngpath;
}
elseif($path == "media") {
	$tngpath = $mediapath;
...
function frmFiles() {
	global $ImageFileTypes, $subdir, $img, $admtext, $text, $page, $rootpath, $path, $tngpath, $fulltngpath, $displaypath, $pagetotal, $searchstring, $allow_delete, $dims, $tngconfig, $session_charset, $folders, $namedir, $datedir, $order, $cms;
	$columns = 4;
...
<?php
	...
	$validpath = false;
	if( file_exists("$fulltngpath/" . stripslashes($subdir)) && is_dir("$fulltngpath/" . stripslashes($subdir)) ) {
		$validpath = true;
	}
	$lRecCount = lCountFiles($validpath);
	$nPages = intval( ( $lRecCount - 0.5 ) / $pagetotal ) + 1;
	$lStartRec = $nCurrentPage * $pagetotal;

	frmFilesHdFt( $columns, $nCurrentPage, $nPages );
?>
<span class="normal">&nbsp;<?php echo "<b>{$admtext['folder']}:</b> $displaypath/" . stripslashes($subdir); ?> 
	&nbsp; | &nbsp; <b><?php echo $admtext['search']; ?>:</b> <input type="text" onkeyup="fpFilterChanged(event, this.value);"></span><br/>
<?php
	if( !$validpath ) {
		// The directory doesn't exist, so let the user know.
		echo "<tr><td><span class=\"normal\" style=\"color:#FF0000\"><em>" . $admtext['cannotopendir'] . " (" . $tngpath . "/" . stripslashes($subdir) . ") " . $admtext['improperpermissions'] . "</em></span></td></tr>";
	}
	$actionwidth = $folders ? 160 : 100;
?>
<table cellpadding="3" cellspacing="1" border="0" width="100%" class="normal">
   <tr class="fieldnameback">
      <td align="left" width="<?php echo $actionwidth; ?>"><span class="fieldname"><b><?php echo $admtext['action']; ?></b></span></td>
...
<?php
	...
	$savedir = getcwd();
	if( file_exists("$fulltngpath/" . stripslashes($subdir)) && is_dir("$fulltngpath/" . stripslashes($subdir)) ) {
		chdir("$fulltngpath/" . stripslashes($subdir));
	}
	if( $handle = @opendir('.') ) {
		$fnentries = array();
		$dnentries = array();
		while ($file = readdir( $handle )) {
			if( !$searchstring || strpos(strtoupper($file),strtoupper($searchstring)) !== false ) {
				if(is_file($file)) {
					if(!$folders) {
		...
		foreach( $nentries as $file => $value ) {
			...
			if( is_file( $filename ) && $filename != "index.html" ) {
				...
				$file_ext = isset($fileparts["extension"]) ? strtoupper( $fileparts["extension"] ) : "";
				...
				echo "<a href=\"javascript:ReturnFile('$img$subdir" . addslashes($file) . "')\" title=\"{$admtext['select']}\" class=\"newsmallericon\"><img src=\"img/check.png\" /></a>";
				echo "<a href=\"javascript:ShowFile('$tngpath/$subdir" . addslashes($file) . "')\" title=\"{$admtext['preview']}\" class=\"newsmallericon\"><img src=\"img/play-circle.png\" /></a>\n";
				if($allow_delete)
					echo "<a href=\"#\" onclick=\"return deleteIt('file','$nImageNr','$tngpath/$subdir" . addslashes($file) . "');\" title=\"{$admtext['text_delete']}\" class=\"newsmallericon\"><img src=\"img/times.png\" /></a>";
?>
            </div>
        </td>
...
?>
			<span class="normal"><a href="#" onclick="return moreFilepicker({subdir: '<?php echo addslashes($newsubdir); ?>',path: '<?php echo $path; ?>',folders: '<?php echo $folders; ?>',order: '<?php echo $order; ?>'});"><img src="img/folder-open.png" class="newsmallicon" /><?php echo $admtext['open']; ?></a></span>
		</td>
...
function lCountFiles($validpath) {
	global $ImageFileTypes, $subdir, $rootpath, $fulltngpath, $searchstring, $folders;
