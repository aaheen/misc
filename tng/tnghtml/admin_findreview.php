...
if( $type == "I" ) {
	...
	if(!empty($newsearch)) {
		...
		if( !isset($offset) ) {
			$page = isset($_SESSION['tng_search_preview_post']['page']) ? $_SESSION['tng_search_preview_post']['page'] : 1;
			$offset = isset($_SESSION['tng_search_preview_post']['offset']) ? $_SESSION['tng_search_preview_post']['offset'] : 0;
		}
		else {
...
<div class="admin-main">
	...
	<table>
		<tr>
			<td><span class="label"><?php echo $admtext['user'];?>:</span></td>
			<td>
				<select class="bigselect" name="reviewuser">
<?php
...
?>
			...
			<td rowspan="2" style="vertical-align: top;padding-left:5px;">
				<input type="submit" name="submit" class="btn"value="<?php echo $admtext['search']; ?>" class="aligntop">
				<input type="submit" name="submit" class="medbtn btn" value="<?php echo $admtext['reset']; ?>" onClick="document.form1.reviewuser.value=''; document.form1.tree.selectedIndex=0; document.form1.living.checked=false; document.form1.exactmatch.checked=false;" class="aligntop">
			</td>
		</tr>
		<tr>
...
<?php
$actionstr = "<a href=\"admin_review.php?tempID=xxx\" title=\"{$admtext['review']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
if( $allow_delete )
...
while( $row = tng_fetch_assoc($result))
{
	...
	echo "<tr id=\"row_{$row['tempID']}\"><td class=\"lightback\" valign=\"top\"><nobr>$newactionstr</nobr></td>\n";
	echo "<td class=\"lightback\" valign=\"top\">&nbsp;$persfamID&nbsp;</td>\n";
	echo "<td class=\"lightback\" valign=\"top\">&nbsp;$name&nbsp;</td>\n";
	echo "<td class=\"lightback\" valign=\"top\">&nbsp;$displayval&nbsp;</td>\n";
	echo "<td class=\"lightback\" valign=\"top\">&nbsp;{$row['postdate']}&nbsp;</td>\n";
	echo "<td class=\"lightback\" valign=\"top\">&nbsp;{$row['treename']}&nbsp;</td></tr>\n";
}
