...
//var formname = "find";
var findform = "find";
var resheremsg = '<span class="normal">' + '</span>'; //can't remember what this was supposed to be
...
function validateForm( ) {
	if( document.find.newlink1.value.length == 0 ) {
		...
		linkStuff('linkerform','masslink');
	return false;
}

function validateForm2( ) {
	if( document.find2.cemeteryID.selectedIndex == 0 ) {
		alert("<?php echo $admtext['selectcemetery']; ?>");
	}
	else
		linkStuff('linkerform2','cemlink');
	return false;
}
...
function linkStuff(form_id, action) {
	var medialist = "";
	...
	if(medialist) {
		...
		params += "&medialist=" + medialist + "&action=" + action;
		jQuery.ajax({
...
<div class="admin-main label">
	...
	<form id="fileupload" action="//jquery-file-upload.appspot.com/" method="POST" enctype="multipart/form-data">
	        <!-- Redirect browsers with JavaScript disabled to the origin page -->
				...
				<select name="mediatypeID" class="bigselect" id="mediatypeID" onchange="changeCollection(this);">
<?php
	foreach( $mediatypes as $mediatype ) {
		$msgID = $mediatype['ID'];
...
?>
				...
				&nbsp;
<?php
	echo "<br/><br/>" . $admtext['tree'] . ": ";
    $treeresult = tng_query($treequery) or die ($admtext['cannotexecutequery'] . ": $treequery");
	if( $assignedtree ) {
		$treerow = tng_fetch_assoc($treeresult);
		echo $treerow['treename'];
		tng_free_result($treeresult);
		echo "<input type=\"hidden\" name=\"tree\" value=\"{$treerow['treename']}\">";
	}
	else {
	...
	echo "<br/>" . $admtext['folder'] . ": <span id=\"folderlabel\">{$mediatypes_assoc[$mediatypeID]}</span>$treestr" . "/";
?>
			<input type="text" id="folder" name="folder" class="medfield"/> <input type="button" class="btn" value="<?php echo $admtext['select'] . "..."; ?>" name="folderselect" onclick="FilePicker('folder',jQuery('#mediatypeID').val(),1);">
			<br/><br/>
        	...
        	<div class="row fileupload-buttonbar" style="float:left">
	        	<div class="span7">
		            ...
		            <span class="btn fileinput-button" style="height:23px">
	...
	<div style="float:left; margin-right: 20px">
		<form action="" method="get" name="find" id="linkerform" onsubmit="return validateForm();">
		<table cellspacing="2">
			<tr>
...
?>
				...
				<td valign="top">
					<select name="linktype1" class="bigselect" onchange="toggleEventLink(this.selectedIndex);">
				...
				<td><a href="#" onclick="return findItem(document.find.linktype1.options[document.find.linktype1.selectedIndex].value,'newlink1',null,document.find.tree1.options[document.find.tree1.selectedIndex].value,'<?php echo $assignedbranch; ?>');" title="<?php echo $admtext['find']; ?>">
					<img src="img/search.png" alt="<?php echo $admtext['find']; ?>" class="alignmiddle" /></a>&nbsp;</td>
				<td>
			    	<input type="button" class="btn toggle" value="<?php echo $admtext['selectall']; ?>">
					<input type="submit" id="linker" class="btn" value="<?php echo $admtext['linksel']; ?>">
				</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td colspan="2">
					<span id="eventlink1"><input type="checkbox" name="eventlink1" value="1" onclick="return toggleEventRow(this.checked);" /> <?php echo $admtext['eventlink']; ?></span><br />
					<select name="event1" class="bigselect" id="eventrow1" style="display:none">
						<option value=""></option>
					</select>
				</td>
				<td valign="top"><span id="linkermsg"></span>&nbsp;</td>
			</tr>
		</table>
		</form>
	</div>
	<div>
		<form action="" method="get" name="find2" id="linkerform2" onsubmit="return validateForm2();">
		<table cellspacing="2">
			<tr>
				<td colspan="3"><?php echo $admtext['cemetery']; ?></td>
			</tr>
			<tr>
				<td valign="top">
					<select name="cemeteryID" class="bigselect" id="cemeteryID" style="max-width:400px">
...
?>
				...
				<td>
			    	<input type="button" class="btn toggle" value="<?php echo $admtext['selectall']; ?>">
					<input type="submit" class="btn" id="linker2" value="<?php echo $admtext['linksel']; ?>">
				</td>
			</tr>
		</table>
		</form>
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-download fade">
        ...
        {% } else { %}
            ...
            {% } %}</td>
			<td>
				<?php echo $admtext['title2'] . "<hr class=\"uploadhr\"/>\n" . $admtext['description']; ?><br/><br/>
				<button class="savebutton btn" id="q{%=file.mediaID%}" disabled="disabled">
            ...
            <td class="size">
            	<button class="linksbutton btn" id="l{%=file.mediaID%}">
        ...
        {% } %}
        <td class="delete">
            <button class="delbutton btn" data-type="{%=file.delete_type%}" data-url="{%=file.delete_url%}"{% if (file.delete_with_credentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %} onclick="return confirmDelete(event);">
