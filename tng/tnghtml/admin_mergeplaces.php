...
if( !empty($place) ) {
	...
	if( !empty($_COOKIE['tng_merge_search_post']['search']) )
		...
		if( !empty($_COOKIE['tng_merge_places_post']['place']) )
			$place = stripslashes($_COOKIE['tng_merge_places_post']['place']);
		if( !empty($_COOKIE['tng_merge_places_post']['place2']) )
...
?>
	...
	<input type="submit" name="submit" class="btn" value="<?php echo $admtext['text_continue']; ?>">
	<input type="submit" name="reset" class="btn" value="<?php echo $admtext['reset']; ?>" onclick="return resetFields()">
	</form>
<?php
	if($place && $numrows) {
?>
    ...
    <p><input type="submit" class="btn" value="<?php echo $admtext['mergeplaces']; ?>"> <img src="img/spinner.gif" id="placespin" style="display:none">
...
<?php
		while( $row = tng_fetch_assoc($result))
		{
            ...
            $display = preg_replace("/>/", "&gt;", $display);
			echo "<td class=\"lightback\" valign=\"top\"><a href=\"admin_editplace.php?ID={$row['place']}{$treestr}&cw=1\" target=\"blank\">$display</a>&nbsp;</td>\n";
...
<?php
	if($numrows > 10) {
?>
	<input type="submit" class="btn" value="<?php echo $admtext['mergeplaces']; ?>">
<?php
	}
?>
