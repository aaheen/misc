...
<?php
	if(!empty($message)) {
?>
	<span class="red"><em><?php echo urldecode($message); ?></em>
	</span>
<?php
	}
?>
...
<table border="0" cellpadding="2">
	<tr><td><?php echo $admtext['eventtype']; ?>:</td>
		<td>
			<select name="eventtypeID" class="bigselect" id="eventtypeID">
				<option value=""></option>
<?php
	$query = "SELECT * FROM $eventtypes_table WHERE keep = \"1\" AND type = \"$prefix\" ORDER BY tag";
...
?>
	...
	<tr><td><?php echo $admtext['eventdate']; ?>:</td><td><input type="text" name="eventdate" onBlur="checkDate(this);"> <?php echo $admtext['dateformat']; ?>:</td></tr>
	<tr><td><?php echo $admtext['eventplace']; ?>:</td><td valign="top"><input type="text" name="eventplace" id="eventplace" class="verylongfield"> &nbsp;<?php echo $admtext['text_or']; ?>&nbsp;
		<a href="#" onclick="return openFindPlaceForm('eventplace');" title="<?php echo $admtext['find']; ?>"><img src="img/search.png" class="alignmiddle" title="<?php echo $admtext['find']; ?>" alt="<?php echo $admtext['find']; ?>" /></a></td></tr>
	...
	<tr>
		...
		<td>
			<table cellpadding="0"><tr><td><input type="text" name="dupIDs" id="dupIDs" class="medfield"> &nbsp;<?php echo $admtext['text_or']; ?>&nbsp;</td>
				<td><a href="#" onclick="return findItem('<?php echo $prefix; ?>','dupIDs','','<?php echo $tree; ?>','<?php echo $assignedbranch; ?>');" title="<?php echo $admtext['find']; ?>"><img src="img/search.png" class="alignmiddle" alt="<?php echo $admtext['find']; ?>" /></a></td></tr>
			</table>
		</td>
	</tr>
	<tr>
