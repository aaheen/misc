...
$wherestr = !$allow_private_notes ? " AND $notelinks_table.secret != \"1\"" : "";
$query = "SELECT $notelinks_table.ID as ID, $xnotes_table.note as note, noteID, secret FROM ($notelinks_table, $xnotes_table)
    WHERE $notelinks_table.xnoteID = $xnotes_table.ID AND $notelinks_table.gedcom = $xnotes_table.gedcom
        AND persfamID=\"$persfamID\" AND $notelinks_table.gedcom =\"$tree\" AND eventID = \"$eventID\" $wherestr ORDER BY ordernum, ID";
...
<p>
<?php if( $allow_add ) { ?>
	<input type="button" class="btn" value="  <?php echo $admtext['addnew']; ?>  " onclick="document.form2.reset();gotoSection('notelist','addnote');" />&nbsp;
<?php } ?>
	<input type="button" class="btn" value="  <?php echo $admtext['finish']; ?>  " onclick="tnglitbox.remove();" />
</p>
		...
		<tr>
			...
			<td class="fieldnameback" width="120"><b><?php echo $admtext['action']; ?></b></td>
...
<?php
	if( $notelinks && $notecount ) {
		...
		while ( $note = tng_fetch_assoc( $notelinks ) ) {
			...
			$citesicon = tng_num_rows($citresult) ? "quotes_on.png" : "quotes.png";
			tng_free_result($citresult);

			$note['note'] = cleanIt($note['note']);
			if($note['secret'])
				$note['note'] = "[{$admtext['private']}] " . $note['note'];
			$truncated = truncateIt($note['note'],75);
			$actionstr = $allow_edit ? "<a href=\"#\" onclick=\"return editNote({$note['ID']});\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>" : "";
			$actionstr .= $allow_delete ? "<a href=\"#\" onclick=\"return deleteNote({$note['ID']},'$persfamID','$tree','$eventID');\" title=\"{$admtext['text_delete']}\" class=\"newsmallericon\"><img src=\"img/times.png\" /></a>" : "";
			$actionstr .= "<a href=\"#\" onclick=\"return showCitationsInside('N{$note['ID']}','{$note['noteID']}', '$persfamID');\" title=\"{$admtext['sources']}\" class=\"newsmallicon\"><img src=\"img/{$citesicon}\" id=\"citesiconN{$note['ID']}\"class=\"newsmallicon\" /></a>";
			...
			echo "<td class=\"lightback dragarea\"><img src=\"img/admArrowUp.gif\" alt=\"\"><br/><img src=\"img/admArrowDown.gif\" alt=\"\"></td>";
			echo "<td class=\"lightback\" width=\"120\">$actionstr</td>";
...
<div style="float:right;text-align:center">
    ...
    <p><a href="#" onclick="gotoSection('addnote','notelist');return false;"><?php echo $text['cancel']; ?></a></p>
</div>
...
<table border="0" cellpadding="2">
	...
	<td><textarea wrap cols="60" rows="25" name="note" id="newnote"></textarea></td></tr>
<?php
