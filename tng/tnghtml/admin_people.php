...
if($order == "change") {
	$orderstr = "$people_table.changedate, lastname, lnprefix, firstname";
	...
	if($order == "changeup")
		$orderstr = "$people_table.changedate DESC, lastname, lnprefix, firstname";
}
...
?>
	...
	<input type="submit" name="submit" class="medbtn btn" value="<?php echo $admtext['reset']; ?>" onclick="resetPeople();" class="aligntop">
...
?>
			<div>
				...
				<input type="button" name="clearall" class="btn bigfield" value="<?php echo $admtext['clearall']; ?>" onClick="toggleAll(0);">
		  		<input type="submit" name="xperaction" class="btn bigfield" value="<?php echo $admtext['deleteselected']; ?>" onClick="return confirm('<?php echo addslashes($admtext['confdeleterecs']); ?>');">
	  		</div>
<?php
	}
?>
...
<?php
	if( $numrows ) {
		$actionstr = "";
		if( $allow_edit )
			$actionstr .= "<a href=\"admin_editperson.php?personID=xxx&amp;tree=yyy\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
		$actionstr .= "<a href=\"" . $getperson_url . "personID=xxx&amp;tree=yyy\" target=\"_blank\" title=\"{$admtext['test']}\" class=\"newsmallericon\"><img src=\"img/play-circle.png\" /></a>";
		if( $allow_delete )
		...
		while( $row = tng_fetch_assoc($result))
		{
			...
			if ( $row['deathdate'] ) {
				...
				$deathdate = $admtext['burialabbr'] . " " . $row['burialdate'];
			...
			if(($allow_private == 0 && $row['private'] == 1) || ($allow_living == 0 && $row['living'] == 1)) {
				$birthdate = $admtext['private'];
				$birthplace = $admtext['private'];
				$deathdate = $admtext['private'];
				$deathplace = $admtext['private'];
				$photostr = "";
			}
			if($birthdate) $birthdate .= "<br/>";
			if($deathdate) $deathdate .= "<br/>";

			echo "<td class=\"lightback\" valign=\"top\">$namestr</td>\n";
			echo "<td class=\"lightback thumbholder\" align=\"center\" valign=\"top\">$photostr</td>\n";
			echo "<td class=\"lightback\" valign=\"top\">$birthdate{$birthplace}</td>\n";
			echo "<td class=\"lightback\" valign=\"top\">$deathdate{$deathplace}</td>\n";
			if($numtrees > 1)
