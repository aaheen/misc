...
<td class="tngshadow databack">
	...
	<table class="label">
		<tr>
			...
			<td>
				<select name="mediatypeID" class="bigselect" onChange="switchOnType(this.options[this.selectedIndex].value)">
<?php
	foreach( $mediatypes as $mediatype ) {
		$msgID = $mediatype['ID'];
...
?>
			<input type="button" name="addnewmediatype" value="<?php echo $admtext['addnewcoll']; ?>" class="btn aligntop" onclick="tnglitbox = new LITBox('admin_newcollection.php?field=mediatypeID',{width:600,height:600});">
			<input type="button" name="editmediatype" id="editmediatype" value="<?php echo $admtext['edit']; ?>" class="btn" style="vertical-align:top;display:none" onclick="editMediatype(document.form1.mediatypeID);">
			<input type="button" name="delmediatype" id="delmediatype" value="<?php echo $admtext['text_delete']; ?>" class="btn" style="vertical-align:top;display:none" onclick="confirmDeleteMediatype(document.form1.mediatypeID);">
<?php
	}
?>
			</td>
		...
		<tr>
			...
			<td>
				<select name="tree" class="bigselect">
					<option value=""><?php echo $admtext['alltrees']; ?></option>
<?php
