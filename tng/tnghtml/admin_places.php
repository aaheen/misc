...
?>
	<input type="text" name="searchstring" value="<?php echo stripslashes($searchstring_noquotes); ?>" class="verylongfield" style="margin-bottom:5px;">
...
?>
			<div>
				...
				<input type="button" name="clearall" class="btn bigfield" value="<?php echo $admtext['clearall']; ?>" onClick="toggleAll(0);">
		  		<input type="submit" name="xplacaction" class="btn bigfield" value="<?php echo $admtext['deleteselected']; ?>" onClick="return confirm('<?php echo addslashes($admtext['confdeleterecs']); ?>');">
	  		</div>
<?php
	}
?>
...
<?php
	if( $numrows ) {
		$actionstr = "";
		if( $allow_edit )
			$actionstr .= "<a href=\"admin_editplace.php?ID=xxx\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
		$actionstr .= "<a href=\"" . $placesearch_url . "psearch=zzz";
		if(!$tngconfig['places1tree'])
			$actionstr .= "&amp;tree=yyy";
		$actionstr .= "\" target=\"_blank\" title=\"{$admtext['test']}\" class=\"newsmallericon\"><img src=\"img/play-circle.png\" /></a>";
		if( $allow_delete )
		...
		while( $row = tng_fetch_assoc($result))
		{
			...
			echo "<td class=\"lightback\">{$row['changedatef']}</td>\n";
			echo "</tr>\n";
		}
?>
	</table>
