...
if( $gedcom ) {
	$recipstr = "gedcom=\"$gedcom\"";
	if( $branch )
		$recipstr = "($recipstr AND branch=\"$branch\")";
	$wherestr = !empty($sendtoadmins) ? " AND ($recipstr OR gedcom=\"\")" : " AND $recipstr";
}
...
if ( !$numrows ) {
	...
	adminwritelog( $admtext['sentmailmessage'] . ": " . $row['realname'] . " ($recipient)" );
