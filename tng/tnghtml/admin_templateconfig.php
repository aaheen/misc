...
while( $row = tng_fetch_assoc( $result ) ) {
	if( is_dir($cms['tngpath'] . "templates/template" . $row['template']) || is_dir($cms['tngpath'] . "templates/" . $row['template'])) {
...
$result = tng_query($query);
$languageList = tng_num_rows($result) ? "<option value=\"\">{$admtext['select']}</option>\n" : "";
while($row = tng_fetch_assoc($result)) {
	$key = $row['folder'];
...
function showUploadBox(key, folder) {
	jQuery('#div_' + key).html("<input type=\"file\" name=\"upload_" + key + "\" onchange=\"populateFileName(this,jQuery('#form_" + key + "'));\"/> <?php echo $admtext['text_or']; ?> <input type=\"button\" class=\"btn\" value=\"<?php echo $admtext['select']; ?>\" name=\"photoselect_" + key + "\" onclick=\"javascript:FilePicker('form_" + key + "','" + folder + "');\" />");
...
<tr>
<td class="tngshadow databack" style="display:flex; gap:40px;">
	<div>
		<form name="formtop1" action="admin_updatetemplateconfig.php" method="post">
			...
			<select class="bigselect" name="form_templateswitching" id="form_templateswitching" onchange="if(jQuery(this).val() == '0') {jQuery('#form_templatenum').val(''); jQuery('#topsave').show(); switchTemplates('')}">
			...
			<input type="submit" name="submittop" id="topsave" class="btn" value="<?php echo $admtext['save']; ?>" style="display:none"/>
			<input type="hidden" name="form_templatenum" value=""/>
		</form>
	</div>
	<div>
		<form name="formtop3" id="formtop3" action="admin_template_importer.php" method="post" ENCTYPE="multipart/form-data"">
			<label for="form_newtemplate"><?php echo $admtext['importtemp']; ?>:</label>
			<input type="file" name="form_newtemplate" id="form_newtemplate" class="importer" size="60">
			<input type="submit" name="submittop" id="topsave2" class="btn" value="<?php echo $admtext['import']; ?>" onclick="if(jQuery('#form_newtemplate').get(0).files.length === 0) {return false;}"/>
		</form>
	</div>
</td>
</tr>
...
?>
		<select class="bigselect" name="form_templatenum" id="form_templatenum" onchange="switchTemplates(jQuery(this).val());">
			<option value=""></option>
<?php
...
?>
		</select> <button id="previewbtn" class="btn" style="margin-top:-3px"><span class="prevmsg"><?php echo $admtext['showprev']; ?></span><span class="prevmsg" style="display:none"><?php echo $admtext['hideprev']; ?></span></button>
	</form>
...
<?php
	...
	$needtrans = array('headline','maintitle','welcome','hisside','herside','headtitle1','headtitle2','headtitle3','momside','dadside','mainpara','featurepara',
		'featurelinks','featurelinktext','searchpara','fhpara','mwpara','respara','headtitle','headsubtitle','latestnews','featuretitle1','featuretitle2','featuretitle3',
		'featuretitle4','featuretitle5','featuretitle6','featuretitle7','featuretitle8','featuretitle9','featurepara1','featurepara2','featurepara3','featurepara4',
		'featurepara5','featurepara6','featurepara7','featurepara8','featurepara9','photocaption','newstext','menutitle','phototitlel','photocaptionl','phototitler',
		'photocaptionr','topsurnames','featurespara','sidebarhead1','sidebarhead2','sidebarhead3','sidebartitle','sidebarsub','sidebarpara','sidebarlinktext',
		'texttitle1','texttitle2','texttitle3','textpara1','textpara2','textpara3','subhead1','subhead2','snipinfoone','snipinfotwo','quote','subhead',
		'ancestors','momheading','mompara','dadheading','dadpara','spmomheading','spmompara','spdadheading','spdadpara','spmomside','spdadside','storiesheading',
		'quotehead','slideheadline','slidetext','slidelinktext','footerheadline','footerlinktext','footerpara','linktext','featuresub','featuretitle','headline',
		'snipheadline','snipname','slidetitle','snipsubhead','sniptitle','headerlinktext','reslinks');
	foreach($tmp as $key=>$value) {
		...
		if( !isset($sections[$n]) ) $sections[$n] = "";
		...
		$sections[$n] .= "<td$align>";
		$sections[$n] .= isset($parts[2]) ? "&nbsp;&nbsp;{$admtext[$label]}{$index}:" : ($label != "" ? $admtext[$label] . $index . ":" : $index . ":");
		$langName = isset($parts[2]) && isset($languageArray[$parts[2]]) ? $languageArray[$parts[2]] : (!empty($parts[2]) ? $parts[2] : "");
		$sections[$n] .= $langName ? "<br/>&nbsp;&nbsp;&nbsp;&nbsp;(" . $langName . ")" : "";
		$sections[$n] .= "</td>\n";
		$sections[$n] .= "<td>";
		if($type == "textarea") {
			$sections[$n] .= "<textarea name=\"form_$key\" id=\"form_$key\" rows=\"5\" cols=\"80\" style=\"margin-bottom:5px\">$value</textarea>\n";
		}
		elseif($type == "select") {
			$sections[$n] .=  "<select class=\"bigselect\" name=\"form_$key\" id=\"form_$key\">\n";
			for( $j = 1; $j <= $treenum; $j++ ) {
		...
		elseif($label == "titlechoice") {
			...
			if(strpos($key, "img") !== false || strpos($key, "image") !== false || strpos($key, "thumb") !== false || strpos($key, "photol") !== false || strpos($key, "photor") !== false) {
				$sections[$n] .= " <input type=\"button\" class=\"btn\" onclick=\"if(jQuery('#form_$key').val()) return preview('templates/{$folders[$n]}/' + jQuery('#form_$key').val());\" value=\"{$admtext['preview']}\" /> <input type=\"button\" class=\"btn\" onclick=\"return showUploadBox('$key','{$folders[$n]}');\" value=\"{$admtext['change']}\" />\n";
				$safevalue = strpos($value, "&#34;") != false ? substr($value, 0, strpos($value, "&#34;")) : $value;
				$size = @GetImageSize($rootpath . "templates/{$folders[$n]}/$safevalue");
				if( $size ) {
					$imagesize1 = $size[0];
					$imagesize2 = $size[1];
			...
			elseif(substr($label, -6) == "person") {
				...
				$sections[$n] .= "<img src=\"img/search.png\" alt=\"{$admtext['find']}\" class=\"alignmiddle\" />\n";
				$sections[$n] .= "</a>\n";
			}
		}
		if($languageList && !isset($parts[2]) && in_array($label,$needtrans)){
			if($type == "textarea")
				$sections[$n] .= "<br />";
			$sections[$n] .= "{$admtext['createcopy']}: \n<select class=\"bigselect\" id=\"lang_$key\">\n$languageList\n</select> <input type=\"button\" class=\"btn\" value=\"{$admtext['go']}\" onclick=\"return insertLangRow('$key','$type');\" />\n";
		}
		$sections[$n] .= "</td>\n</tr>\n";
	}
	//debugPrint($sections);
	foreach($entries as $i) {
		...
		if($section) {
			...
			echo "<table class=\"tstable label\">\n";
			$newtemplatepfx = is_numeric($i) ? "template" : "";
			$imagetext = "";
			...
			if($imagetext)
				echo "$imagetext<br />";
			echo "<input type=\"submit\" name=\"submit{$i}\" class=\"btn saveret\" accesskey=\"s\" value=\"{$admtext['saveback']}\">\n";
			echo "<input type=\"submit\" name=\"submitx{$i}\" class=\"btn savestay\" accesskey=\"s\" value=\"{$admtext['savereturn']}\">\n";
