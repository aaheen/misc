		...
		if( substr( $key, 0, 2 ) == "ph" ) {
			...
			if( $oldmediatype != $newmediatype ) {
				...
				if( $row['usecollfolder'] ) {
					...
					if( $row['path'] ) {
						$oldpath = "$rootpath$oldmediapath/$treestr" . $row['path'];
						$newpath = "$rootpath$newmediapath/$treestr" . $row['path'];
						@rename($oldpath, $newpath);
					}

					if( $row['thumbpath'] ) {
						$oldthumbpath = "$rootpath$oldmediapath/$treestr" . $row['thumbpath'];
						$newthumbpath = "$rootpath$newmediapath/$treestr" . $row['thumbpath'];
