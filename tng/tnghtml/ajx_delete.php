...
switch($t) {
	...
	case "media":
		...
		resortMedia($id);
		if(!empty($confirm) || $tngconfig['mediadel'] == 1)
			removeImages($id);
	...
	case "tlevent":
		...
		$logmsg = $admtext['tlevents'] . ": ID $id {$admtext['succdeleted']}";
		break;
	case "note":
		...
		$logmsg = $admtext['notes'] . ": ID $id {$admtext['succdeleted']}";
		break;
	case "person":
		if(!empty($id)) {
			...
			$personID = $row['personID'];
			$tree = $row['gedcom'];
		} elseif(!empty($personID) && !empty($tree)) {
			$query = "SELECT ID, branch, sex FROM $people_table WHERE personID=\"$personID\" AND gedcom=\"$tree\"";
			$result = @tng_query($query);
			$row = tng_fetch_assoc($result);
			$id = $row['ID'];
			tng_free_result($result);
	...
	case "tree":
		...
		$tquery = "DELETE FROM $trees_table WHERE gedcom=\"$thisid\"";
	...
	case "dna":
		...
		break;
	case "msg":
		$query = "DELETE FROM $templates_table WHERE id=\"$id\"";
		$result = @tng_query($query);
		break;
}
if($logmsg)
