	...
	if( $personID) {
		...
		if($row) {
  		$rights = determineLivingPrivateRights($row, true, true);
			...
			$names .= "<a href=\"admin_editperson.php?personID={$personID}&amp;tree=$tree&amp;cw=1\" target=\"blank\">" . getName($row) . " ($personID)</a><br/>\n";
		}
...
function setFamilyLabel( $personID, $gender ) {
	global $tree, $families_table, $branch, $admtext, $overwrite, $branchlinks_table, $children_table, $branchaction, $people_table, $fdone, $famnames;
	...
	if( $gender['self'] ) {
		...
		while($row = tng_fetch_assoc( $result )) {
			...
			if(!in_array($row['familyID'],$fdone))
				$famnames .= "<a href=\"admin_editfamily.php?familyID={$row['familyID']}&amp;tree={$row['gedcom']}&amp;cw=1\" target=\"blank\">" . getFamilyName($row) . "</a><br/>\n";
			if($branchaction == "delete") {
				...
				$query = "DELETE FROM $children_table WHERE ID=\"{$row['familyID']}\" AND gedcom = \"$tree\"";
				$result2 = @tng_query($query);

				deleteEvents($row['familyID'],$tree);
				deleteCitations($row['familyID'],$tree);
				deleteNoteLinks($row['familyID'],$tree);
				deleteBranchLinks($row['familyID'],$tree);
				deleteMediaLinks($row['familyID'],$tree);
				deleteAlbumLinks($row['familyID'],$tree);
				doFCounter();
			}
