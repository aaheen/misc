...
if ( $parents && tng_num_rows( $parents ) ) {
	while ( $parent = tng_fetch_assoc( $parents ) )
	{
		...
		$gotfather = getParentData($tree, $parent['familyID'], "husband");
			...
			if( !empty($fathrow['firstname']) || !empty($fathrow['lastname']) ) {
		...
		if( $gotmother ) {
			...
			if( !empty($mothrow['firstname']) || !empty($mothrow['lastname']) ) {
...
while ( $marriagerow =  tng_fetch_assoc( $marriages ) )
{
	...
	if( $marriagerow['marrtype'] ) {
		if( empty($stdexf['MARR']) || !is_array( $stdexf['MARR'] ) ) $stdexf['MARR'] = array();
	...
	if( !empty($spouserow['allow_living']) )
	...
	if( $children && tng_num_rows( $children ) ) {
		...
		{
			$ifkids = $child['haskids'] ? ($child['haskids'] == 1 ? "<strong>+</strong>" : $child['haskids']) : "&nbsp;";
