...
function createtables($collation, $table_prefix) {
...
switch( $_POST['subroutine'] ) {
	...
	case 'settings':
		...
		if( $link ) {
			...
			$msg = $text['noconn'] . " Host Name, Database Name, Database Username, Database Password (and possibly the port or socket, if those are required).";
	...
	case 'tables':
		...
		$database_port = !empty($database_port) ? trim($database_port) : null;
		$database_socket = !empty($database_socket) ? trim($database_socket) : null;
		$link = tng_connect($database_host, $database_username, $database_password, $database_name, $database_port, $database_socket);
		if( $link && tng_select_db($link, $database_name)) {
			$badtables = createtables($collation,$table_prefix ?? "");
			if( !$badtables ) {
				$msg = $text['tablescr'];
				$class = "green";
			}
	...
	case 'user':
		...
		$query = "INSERT IGNORE INTO $users_table (description,username,password,password_type,realname,phone,email,website,address,city,state,zip,country,notes,gedcom,personID,role,allow_edit,allow_add,tentative_edit,allow_delete,allow_lds,allow_living,allow_private,allow_private_notes,allow_ged,allow_pdf,allow_profile,branch,dt_activated,no_email) ";
		$query .= "VALUES (\"Administrator\",\"$username\",\"$password\",\"$password_type\",\"$realname\",\"\",\"$email\",\"\",\"\",\"\",\"\",\"\",\"\",\"\",\"\",\"\",\"admin\",\"1\",\"1\",\"0\",\"1\",\"1\",\"1\",\"1\",\"1\",\"1\",\"1\",\"1\",\"\",\"$today\",\"0\")";
		$result = @tng_query($query);
    ...
    case 'outmail':
        $tngconfig['sendingemail'] = $sendingemail;
        $tngconfig['mailhost'] = $mailhost;
        $tngconfig['mailuser'] = $mailuser;
        $tngconfig['mailpass'] = $mailpass;
        $tngconfig['mailport'] = $mailport;
        $tngconfig['mailenc'] = $mailenc;
        $usesmtp = 1;
        $saveconfig = 1;
		$msg = $text['infosaved2'];
		$class = "green";
...
function stripAccents($str) {
    return strtr(tng_utf8_decode($str), tng_utf8_decode('?��?��?�?�?��???��?�?�?��??��?��?�?�?��???��?�?�?��'), 'aaaaaceeeeiiiinooooouuuuyyAAAAACEEEEIIIINOOOOOUUUUY');
}
...
if( $saveconfig ) {
	...
	fwrite( $fp, "\$site_desc = \"\";\n" );
	fwrite( $fp, "\$tngconfig['doctype'] = \"<!DOCTYPE html PUBLIC \\\"-//W3C//DTD XHTML 1.0 Transitional//EN\\\">\";\n" );
	fwrite( $fp, "\$language = \"$language\";\n" );
	fwrite( $fp, "\$charset = \"$charset\";\n" );
	fwrite( $fp, "\$norels = \"\";\n" );
	fwrite( $fp, "\$maxsearchresults = \"$maxsearchresults\";\n" );
	fwrite( $fp, "\$tngconfig['searchorder'] = \"{$tngconfig['searchorder']}\";\n" );
	...
	fwrite( $fp, "\$tngconfig['menu_surnames'] = \"{$tngconfig['menu_surnames']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_firstnames'] = \"{$tngconfig['menu_firstnames']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_psearch'] = \"{$tngconfig['menu_psearch']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_fsearch'] = \"{$tngconfig['menu_fsearch']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_ssearch'] = \"{$tngconfig['menu_ssearch']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_places'] = \"{$tngconfig['menu_places']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_dates'] = \"{$tngconfig['menu_dates']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_cal'] = \"{$tngconfig['menu_cal']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_cems'] = \"{$tngconfig['menu_cems']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_bmarks'] = \"{$tngconfig['menu_bmarks']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_wnew'] = \"{$tngconfig['menu_wnew']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_mostw'] = \"{$tngconfig['menu_mostw']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_reports'] = \"{$tngconfig['menu_reports']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_stats'] = \"{$tngconfig['menu_stats']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_trees'] = \"{$tngconfig['menu_trees']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_branches'] = \"{$tngconfig['menu_branches']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_notes'] = \"{$tngconfig['menu_notes']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_sources'] = \"{$tngconfig['menu_sources']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_repos'] = \"{$tngconfig['menu_repos']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_dna'] = \"{$tngconfig['menu_dna']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_admin'] = \"{$tngconfig['menu_admin']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_log'] = \"{$tngconfig['menu_log']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_contact'] = \"{$tngconfig['menu_contact']}\";\n" );
	fwrite( $fp, "\$tngconfig['menu_profile'] = \"{$tngconfig['menu_profile']}\";\n" );"1";
	fwrite( $fp, "\$tngconfig['istart'] = \"{$tngconfig['istart']}\";\n" );
	fwrite( $fp, "\$tngconfig['showicons'] = \"{$tngconfig['showicons']}\";\n" );
	fwrite( $fp, "\$tngconfig['showhome'] = \"{$tngconfig['showhome']}\";\n" );
	fwrite( $fp, "\$tngconfig['showsearch'] = \"{$tngconfig['showsearch']}\";\n" );
	fwrite( $fp, "\$tngconfig['searchchoice'] = \"{$tngconfig['searchchoice']}\";\n" );
	fwrite( $fp, "\$tngconfig['showlogin'] = \"{$tngconfig['showlogin']}\";\n" );
	fwrite( $fp, "\$tngconfig['showshare'] = \"{$tngconfig['showshare']}\";\n" );
	fwrite( $fp, "\$tngconfig['showprint'] = \"{$tngconfig['showprint']}\";\n" );
	fwrite( $fp, "\$tngconfig['showbmarks'] = \"{$tngconfig['showbmarks']}\";\n" );
	fwrite( $fp, "\$tngconfig['hidechr'] = \"0\";\n" );
	fwrite( $fp, "\$tngconfig['altbirth'] = \"{$tngconfig['altbirth']}\";\n" );
	fwrite( $fp, "\$tngconfig['collapsestd'] = \"{$tngconfig['collapsestd']}\";\n" );
	fwrite( $fp, "\$tngconfig['collstdevents'] = \"{$tngconfig['collstdevents']}\";\n" );
	...
	fwrite( $fp, "\$tngconfig['edit_timeout'] = \"{$tngconfig['edit_timeout']}\";\n" );
	...
	fwrite( $fp, "\$tngconfig['sendingemail'] = \"{$tngconfig['sendingemail']}\";\n" );
	...
	fwrite( $fp, "\$tngconfig['mailhost'] = \"{$tngconfig['mailhost']}\";\n" );
	fwrite( $fp, "\$tngconfig['mailuser'] = \"{$tngconfig['mailuser']}\";\n" );
	fwrite( $fp, "\$tngconfig['mailpass'] = \"{$tngconfig['mailpass']}\";\n" );
	fwrite( $fp, "\$tngconfig['mailport'] = \"{$tngconfig['mailport']}\";\n" );
	fwrite( $fp, "\$tngconfig['mailenc'] = \"{$tngconfig['mailenc']}\";\n" );
	...
	fwrite( $fp, "\$tngconfig['docthumbs'] = \"{$tngconfig['docthumbs']}\";\n" );
	fwrite( $fp, "\$tngconfig['histhumbs'] = \"{$tngconfig['histhumbs']}\";\n" );
...
if( $saveimportconfig ) {
	...
	fwrite( $fp, "\$tngimpcfg['saveconfig'] = \"\";\n" );
...
echo "</installElement>\n";
echo "<message>\n";
echo "<messageText>" . stripAccents($msg) . "</messageText>\n";
echo "</message>\n";
echo "</install>\n";
?>