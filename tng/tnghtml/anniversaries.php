...
$tngevent = isset($tngevent) ? preg_replace("/[^A-Za-z0-9_\-. ]/", '', $tngevent) : "";
if(!isset($tngliving)) $tngliving = 0;
...
if( empty($tngneedresults) ) {
	...
	$tngmonth = isset($tngmonth) ? preg_replace("/[^0-9]/", "", $tngmonth ) : "";
	$tngdaymonth = isset($tngdaymonth) ? preg_replace("/[^0-9]/", "", $tngdaymonth ) : "";
	$tngneedresults = isset($tngneedresults) ? preg_replace("/[^0-9]/", '', $tngneedresults) : "";
}
...
<?php
	...
	echo "<option value=\"crem\"";
	if( $tngevent == "crem" ) echo " selected=\"selected\"";
	echo ">{$text['cremated']} ({$text['buried']})</option>\n";
...
<div class="annfield">
	...
	<input type="text" name="tngyear" id="tngyear" size="6" maxlength="4" class="bigfield" value="<?php echo $tngyear; ?>" />
</div>
<div class="annfield">
	<label for="tngkeywords"><?php echo $text['keyword']; ?>:</label><br/>
	<input type="text" name="tngkeywords" id="tngkeywords" size="20" class="bigfield" value="<?php echo stripslashes($tngkeywordsclean); ?>" />
</div>
<?php
	if($allow_living) {
?>
<div class="annfield">
	<label for="tngliving"><?php echo $text['livingonly']; ?>:</label><br/>
	<select name="tngliving" id="tngliving" style="max-width:335px" class="bigselect">
		<option value="0" <?php if($tngliving == "0") echo "selected"; ?>><?php echo $admtext['no']; ?></option>
		<option value="1" <?php if($tngliving == "1") echo "selected"; ?>><?php echo $admtext['yes']; ?></option>
	</select>
</div>
<?php
	}
...
<div class="annfield">
	<br/>
	<input type="hidden" name="tngneedresults" value="1" /><input type="submit" class="btn" value="<?php echo $text['search']; ?>" />
	<input type="button" class="btn" value="<?php echo $text['tng_reset']; ?>" onclick="resetForm();" />
	| <input type="button" class="btn" value="<?php echo $text['calendar']; ?>" onclick="window.location.href='<?php echo "{$calendar_url}m=$tngmonth&amp;year=$tngyear&amp;tree=$tree"; ?>';" />
</div>
...
if( $tngneedresults ) {
	...
	if( $tngevent )
		...
		$tngevents = array("birth","altbirth","death","burial","crem","marr","div");
		if($ldsOK) {
	...
	foreach( $tngevents as $tngevent ) {
		...
		switch( $tngevent ) {
			...
			case "burial":
				...
				break;
			case "crem":
				$datetxt = $text['cremated'];
				$tngevent = "burial";
				$burialtypestr = " AND burialtype=1";
				break;
			case "marr":
		...
		$allwhere .= $burialtypestr;

		if( $tngliving && $allow_living ) {
			$livingpeoplewhere = " AND living=\"1\"";
			$livingfamilieswhere = " AND $families_table.living=\"1\"";
		} else {
			$livingpeoplewhere = $livingfamilieswhere = "";
		}
		...
		if($needfamilies) {
			...
			$query = "SELECT $families_table.ID, husb.personID as hpersonID, husb.lastname as hlastname, husb.lnprefix as hlnprefix, husb.firstname as hfirstname,
				...
				WHERE $families_table.gedcom = $trees_table.gedcom $allwhere$livingfamilieswhere ";
			...
			$query = "SELECT $people_table.ID, $people_table.personID, lastname, lnprefix, firstname, $people_table.living, $people_table.private, $people_table.branch, prefix, suffix, nameorder, title, birthdate, birthdatetr, altbirthdate, altbirthdatetr, deathdate, deathdatetr, $place, $datefield, $people_table.gedcom, treename $familiessortdate $eventsfields
				...
				WHERE $people_table.gedcom = $trees_table.gedcom $allwhere$livingpeoplewhere ";
...
<?php
			...
			while( $row = tng_fetch_assoc($result))
			{
				...
				if($rights['both']) {
					...
					$placetxt = $row[$place] ? $row[$place] . " <a href=\"$placesearch_url" . "{$treestr}psearch=" . urlencode( $row[$place] ) . "\" title=\"{$text['findplaces']}\"><img src=\"{$cms['tngpath']}img/tng_search_small.gif\" class=\"placeimg\" border=\"0\" alt=\"{$text['findplaces']}\" width=\"9\" height=\"9\" /></a>" : $info;
