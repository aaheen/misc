...
function doMediaSearch( $instance, $pagenav ) {
	...
	$str .= "<input type=\"text\" class=\"longfield\" name=\"mediasearch\" value=\"$mediasearch\" />\n";
	$str .= "<input type=\"hidden\" name=\"tree\" value=\"$tree\" />\n";
	$str .= "<input type=\"submit\" class=\"btn\" value=\"{$text['search']}\" style=\"margin-bottom:0px\" />\n";
	$str .= "<input type=\"button\" class=\"btn\" value=\"{$text['tng_reset']}\" style=\"margin-bottom:0px\" onclick=\"window.location.href='$browsealbums_noargs_url';\" />&nbsp;&nbsp;&nbsp;";
	$str .= $pagenav;
	$str .= "</form>\n";
...
if($sitever != "standard") {
	...
	$header = "<table cellpadding=\"3\" cellspacing=\"1\"$tablewidth border=\"0\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"$tabletype\"{$headerr}>\n";
} else {
	$header = "<table cellpadding=\"3\" cellspacing=\"1\"$tablewidth border=\"0\" class=\"thfixed whiteback\">";
}
echo $header;
...
$header .= "<th data-tablesaw-priority=\"3\" class=\"fieldnameback fieldname\" style=\"width:70px\">&nbsp;{$text['numitems']}&nbsp;</th>\n";
...
while( $row = tng_fetch_assoc( $result ) ) {
	...
	$albumtext .= "<tr><td valign=\"top\" class=\"databack\">$i</td>";

	$description = $row['description'];
	if($showAlbumInfo) {
		$imgsrc = getAlbumPhoto($row['albumID'],$row['albumname']);
		$alblink = "<a href=\"$showalbum_url" . "albumID={$row['albumID']}\" class=\"media-title\">{$row['albumname']}</a>";
	}
	else {
		$imgsrc = "";
		$alblink = "<span class=\"media-title\">{$text['living']}</span>";
	...
	$albumtext .= "<td class=\"databack\" valign=\"top\">$alblink<br />$description&nbsp;</td>\n";
	$albumtext .= "<td class=\"databack\" valign=\"top\" align=\"center\">{$arow['acount']}&nbsp;</td>\n";
	$albumtext .= "<td valign=\"top\" class=\"databack\" width=\"200\">\n$medialinktext&nbsp;</td>\n";
	$albumtext .= "</tr>\n";
	$i++;
}
