...
function doRepoSearch( $instance, $pagenav ) {
	...
	$str .= "<input type=\"text\" name=\"reposearch\" value=\"$reposearch\" class=\"longfield\" /> <input type=\"submit\" class=\"btn\" value=\"{$text['search']}\" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	$str .= $pagenav;
	if( $reposearch )
...
if ($sitever != "standard") {
	if ($tabletype == "toggle") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" style=\"width:100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"columntoggle\"{$headerr}>\n";
	} elseif ($tabletype == "stack") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" style=\"width:100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"stack\"{$headerr}>\n";
	} elseif ($tabletype == "swipe") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" style=\"width:100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"swipe\"{$headerr}>\n";
	}
} else {
...
?>
	<thead>
		...
		<?php if( $numtrees > 1 ) { ?><th data-tablesaw-priority="3" class="fieldnameback fieldname">&nbsp;<?php echo $text['tree']; ?>&nbsp;</th><?php } ?>
		</tr>
	</thead>
<?php
$i = $offsetplus;
while( $row = tng_fetch_assoc( $result ) )
{
	...
	echo "<td valign=\"top\" class=\"databack\"><a href=\"$showrepo_url" . "repoID={$row['repoID']}&amp;tree={$row['gedcom']}\">{$row['repoID']}</a>&nbsp;</td>";
	echo "<td valign=\"top\" class=\"databack\"><a href=\"$showrepo_url" . "repoID={$row['repoID']}&amp;tree={$row['gedcom']}\">{$row['reponame']}</a>&nbsp;</td>";
	if( $numtrees > 1 )
		echo "<td valign=\"top\" class=\"databack nw\">{$row['treename']}&nbsp;</td>";
	echo "</tr>\n";
	$i++;
}
