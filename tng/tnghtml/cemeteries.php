...
$flags['css'] = "<link href=\"{$cms['tngpath']}css/cemeteries.css?v=" . (filemtime($rootpath."css/cemeteries.css")) . "\" rel=\"stylesheet\" type=\"text/css\" />\n";
...
$colsize = $sitever == "mobile" ? 9999 : DEFAULT_COLUMN_LENGTH;
...
while( $i < $numrows ) {
	if( $cemetery['country'] == $lastcountry ) {
		if($cemetery['state'] == $laststate) {
			if($cemetery['county'] == $lastcounty ) {
				...
				while ( ( $i < $numrows ) && ( $cemetery['county'] == $lastcounty) && ( $cemetery['state'] == $laststate) && ( $cemetery['country'] == $lastcountry) ) { // display all cemeteries in the current county
					if($cemetery['city'] != $lastcity) {
						...
						if($cemetery['city'] || !$tngconfig['cemblanks']) {
							...
							echo "<div class=\"pad3\"><img src=\"" . $cms['tngpath'] . "img/tng_expand.gif\" class=\"expandicon\" title='{$text['expand']}' id='plusminus$divname' onclick=\"return toggleCemSection('$divname');\" alt=\"\" />\n";
							echo "<a href=\"#\" onclick=\"return toggleCemSection('$divname');\" class=\"nounderline\">$txt</a>  &nbsp; ";
							$titletxt = $text['cemeteriesheadstones'] . " " . $text['in'] . " " .$txt;
							echo "<a href=\"$headstones_url" . "country=" . urlencode($cemetery['country']) . "&amp;state=" . urlencode($cemetery['state']) . "&amp;county=" . urlencode($cemetery['county']) . "&amp;city=" . urlencode($cemetery['city']) . "&amp;tree=$tree\" class=\"nounderline\" title=\"$titletxt\"><img src=\"img/headstones.png\" class=\"headstones-icon\" /></a></div>\n";
					...
					$txt= @htmlspecialchars($txt, ENT_QUOTES, $session_charset);
	   				echo "&#x2022; <a href=\"$showmap_url" . "cemeteryID={$cemetery['cemeteryID']}&amp;tree=$tree\" class=\"nounderline\">$txt</a><br/>\n";
					$cemetery = tng_fetch_assoc( $cemresult );
					$i++;
				}
				if($lastcity != DUMMYPLACE)
				...
				if($cemetery['county'] || !$tngconfig['cemblanks']) {
					$linectr++;
                	$txt = $cemetery['county'] ? $cemetery['county'] : $text['nocounty'];
					$titletxt = $text['cemeteriesheadstones'] . " " . $text['in'] . " " .$txt;
					echo "<div class=\"pad3\"><img src=\"" . $cms['tngpath'] . "img/tng_expand.gif\" class=\"expandicon\" title='{$text['expand']}' id='plusminus$divname' onclick=\"return toggleCemSection('$divname');\" alt=\"\" />\n";
                    echo "<a href=\"#\" onclick=\"return toggleCemSection('$divname');\" class=\"nounderline\">$txt</a>  &nbsp; ";
					echo "<a href=\"$headstones_url" . "country=" . urlencode($cemetery['country']) . "&amp;state=" . urlencode($cemetery['state']) . "&amp;county=" . urlencode($cemetery['county']). "&amp;tree=$tree\" class=\"nounderline\" title=\"$titletxt\"><img src=\"img/headstones.png\" class=\"headstones-icon\" /></a></div>\n";
					echo "<div id=\"$divname\" class=\"cemblock\" style=\"display:none;\">\n";
					$hiding = true;
				}
				else {
			...
			$hiding = false;
			$txt = $cemetery['state'] ? $cemetery['state'] : $text['nostate'];
			if($cemetery['state'] || !$tngconfig['cemblanks']) {
				$linectr+=2;        //Add extra line to allow for the <br/> at the end
				echo "<br/><strong><a href=\"$headstones_url" . "country=" . urlencode($cemetery['country']) . "&amp;state=" . urlencode($cemetery['state']) . "&amp;tree=$tree\" class=\"nounderline\">$txt</a></strong><br/>\n";
                echo "<hr/>\n";
			}
			else {
    			$linectr++;
		...
		echo "<div class=\"databack cemcountry subhead rounded10\"><strong><a href=\"$headstones_url" . "country=" . urlencode($cemetery['country']) . "&amp;tree=$tree\" class=\"nounderline\">$txt</a></strong></div>\n";
		$orphan = true;
	}
}
...
if ( $numhs )
    echo "<br/><div class=\"databack cemcountry subhead rounded10\"><strong><a href=\"$headstones_url" . "&amp;tree=$tree\" class=\"nounderline\">{$text['nocemetery']}</a></strong></div>\n";
