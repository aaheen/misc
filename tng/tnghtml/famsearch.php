...
parse_str($_SERVER['QUERY_STRING'], $currargs);
unset($currargs['order']);
$currargs = http_build_query($currargs);

$mybooltext = $mybool == "AND" ? $text['cap_and'] : $text['cap_or'];
$fnameorderstr = $tngconfig['searchorder'] ? "father.firstname, father.lastname" : "father.lastname, father.firstname";
$fnameorderdescstr = $tngconfig['searchorder'] ? "father.firstname DESC, father.lastname DESC" : "father.lastname DESC, father.firstname DESC";
$mnameorderstr = $tngconfig['searchorder'] ? "mother.firstname, mother.lastname" : "mother.lastname, mother.firstname";
$mnameorderdescstr = $tngconfig['searchorder'] ? "mother.firstname DESC, mother.lastname DESC" : "mother.lastname DESC, mother.firstname DESC";

if($order == "marr") {
	$orderstr = "marrdatetr, marrplace, $fnameorderstr";
	...
	if($order == "marrup")
		$orderstr = "marrdatetr DESC, marrplace DESC, $fnameorderstr";
}

if($order == "div") {
	$orderstr = "divdatetr, divplace, $fnameorderstr, marrdatetr";
	...
	if($order == "divup")
		$orderstr = "divdatetr DESC, divplace DESC, $fnameorderstr , marrdatetr";
}

if($order == "fname") {
	$orderstr = "$fnameorderstr, marrdatetr";
	...
	if($order == "fnameup")
		$orderstr = "$fnameorderdescstr, marrdatetr";
}

if($order == "mname") {
	$orderstr = "$mnameorderstr, marrdatetr";
	...
	if($order == "mnameup")
		$orderstr = "$mnameorderdescstr, marrdatetr";
}
...
if( $myfamilyid ) {
	...
	if($fidqualify == "equals" && is_numeric($myfamilyid)) $myfamilyid = $tngconfig['familyprefix'] . $myfamilyid . $tngconfig['familysuffix'];
...
while( $row = tng_fetch_assoc($result))
{
	...
	$fname = $tngconfig['searchorder'] ? getName($frow) : getNameRev($frow);
	$mname = $tngconfig['searchorder'] ? getName($mrow) : getNameRev($mrow);
