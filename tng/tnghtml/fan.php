...
tng_header( "{$text['fanchart']}: {$namestr}  Generations: {$generations}",$flags);
...
for ($a=0; $a < $generations; $a++)
{
	for($b=0; $b < pow(2,$a); $b++)
	{
		if(!empty($perID[pow(2,$a)+$b-1]))
		{
			...
			$perName[$tID]= getName($row, "", true, 4);
			...
			if($rights['both']) {
				...
				$years .= "\"" . strip_tags(justYears($row)) . "\";\n";
			} else {
				$years .= "\"\";\n";
			}
