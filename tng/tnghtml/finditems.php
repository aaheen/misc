...
$criteria = addslashes(trim($criteria));
$f = $filter == "c" ? "%" : "";
...
switch($type) {
	case "I":
		$myffirstname = addslashes(trim($myffirstname));
		$myflastname = addslashes(trim($myflastname));
		...
		$query = "SELECT personID, lastname, firstname, lnprefix, birthdate, birthdatetr, altbirthdate, altbirthdatetr, deathdate, burialdate, title, prefix, suffix, nameorder, living, private, branch, gedcom FROM $people_table WHERE $allwhere ORDER BY lastname, lnprefix, firstname, birthdatetr, altbirthdatetr LIMIT 250";
        ...
        if(tng_num_rows($result)) {
            ...
            while( $row = tng_fetch_assoc($result)) {
                ...
                $lines .= "<td class=\"lightback\"><a href=\"#\" onclick=\"return retItem('{$row['personID']}','I');\" id=\"item_{$row['personID']}\">$namestr</a>&nbsp;</td>\n";
	...
	case "F":
        ...
        if(tng_num_rows($result)) {
            ...
            while( $row = tng_fetch_assoc($result)) {
                ...
                $lines .= "<td class=\"lightback\"><a href=\"#\" onclick=\"return retItem('{$row['familyID']}','F');\" id=\"item_{$row['familyID']}\">$thishusb</a>&nbsp;</td>\n";
	...
	case "S":
        ...
        if(tng_num_rows($result)) {
            ...
            $lines .= "<td class=\"fieldnameback fieldname nw\">&nbsp;<b>" . $admtext['title2'] . "</b>&nbsp;</td>\n";
            $lines .= "</tr>\n";
            ...
            while( $row = tng_fetch_assoc($result)) {
                ...
                $lines .= "<td class=\"lightback\"><a href=\"#\" onclick=\"return retItem('{$row['sourceID']}','S');\" id=\"item_{$row['sourceID']}\">" . truncateIt($title,100) . "</a>&nbsp;</td></tr>\n";
            }
        }
		break;
	case "C":
        ...
        if(tng_num_rows($result)) {
            ...
            while( $row = tng_fetch_assoc($result)) {
				...
				$title = $row['title'] ? $row['title'] : $row['shorttitle'];
	            $lines .= "<td class=\"lightback\">&nbsp;" . $row['sourceID'] . "&nbsp;</td>\n";
                $lines .= "<td class=\"lightback\"><a href=\"#\" onclick=\"return retItem('{$row['citationID']}','C');\" id=\"item_{$row['citationID']}\">" . truncateIt($title,100) . "</a>&nbsp;</td>\n";
    ...
    case "R":
        ...
        if(tng_num_rows($result)) {
            ...
            while( $row = tng_fetch_assoc($result)) {
                ...
                $lines .= "<td class=\"lightback\"><a href=\"#\" onclick=\"return retItem('{$row['repoID']}','R');\" id=\"item_{$row['repoID']}\">" . truncateIt($row['reponame'],75) . "</a>&nbsp;</td></tr>\n";
            }
        }
        break;
    case "L":
        ...
        if(tng_num_rows($result)) {
            ...
            while( $row = tng_fetch_assoc($result)) {
        		...
        		$lines .= "<td class=\"lightback\"><a href=\"#\" onclick='return retItem(\"{$row['ID']}\",\"L\",true);' class=\"rplace\" id=\"item_{$row['ID']}\">{$row['place']}</a>$notes&nbsp;</td></tr>\n";
				$num++;
            }
        }
        break;
	case "G":
		$allwhere = " AND cemname LIKE \"$f$criteria%\"";
		$query = "SELECT cemeteryID, cemname, city, county, state, country 
			FROM $cemeteries_table 
			WHERE cemname like \"$f$criteria%\" OR city like \"$f$criteria%\" OR county like \"$f$criteria%\" OR state like \"$f$criteria%\" or country like \"$f$criteria%\"
			ORDER BY cemname LIMIT 250";
		$result = tng_query($query);

		if(tng_num_rows($result)) {
			$lines = "<tr>\n";
			$lines .= $selectline;
			$lines .= "<td class=\"fieldnameback fieldname nw\">&nbsp;<b>" . $admtext['cemetery'] . "</b>&nbsp;</td>\n";
			$lines .= "</tr>\n";
			...
			while( $cemrow = tng_fetch_assoc($result)) {
				...
				$cemname = preg_replace("/'/","&#39;", $cemname );
					//$cem_slashed = addslashes(preg_replace("/[^A-Za-z0-9]/","_",$cemname));
				$lines .= "<tr id=\"linkrow_{$cemrow['cemeteryID']}\">\n";
				$lines .= "<td class=\"lightback\"><a href=\"#\" onclick='return retCemetery(\"{$cemrow['cemeteryID']}\",\"{$cemname}\");' class=\"rplace\" id=\"item_{$cemrow['cemeteryID']}\">{$cemname}</a>&nbsp;</td></tr>\n";
				$num++;
			}
		}
		break;
...
if(tng_num_rows($result))
    echo "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"660\" class=\"normal\">\n$lines\n</table>\n";
else
