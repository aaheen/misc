...
if( $result ) {
	...
	while( $firstname = tng_fetch_assoc( $result ) ) {
		...
		if( $firstchar == "" )
			$linkstr .= "<a href=\"$search_url" . "myfirstname=$nofirstname&amp;fnqualify=startswith&amp;mybool=AND$treestr\" class=\"snlink\">" . $text['nofirstname'] . "</a> ";
		else {
...
if( $result ) {
	...
	while( $firstname = tng_fetch_assoc( $result ) ) {
		...
		$linkstr2col[$col] .= "<tr><td class=\"snline\" align=\"right\">$count.</td><td class=\"nw\"><a href=\"$search_url" . "myfirstname=$firstname2&amp;fnqualify=startswith&amp;mybool=AND$treestr\">{$firstname['lowername']}</a> <span class=\"lighttext\">($tally_fmt)</span></td>$chartstr</tr>";
		$count++;
	}
	tng_free_result($result);
}
...
?>
			<input type="text" name="topnum" value="100" size="5" maxlength="5" /> <?php echo $text['byoccurrence']; ?>  <input type="hidden" name="tree" value="<?php echo $tree; ?>" /><input type="submit" class="btn" value="<?php echo $text['go']; ?>" /></form>
		</td>
	</tr>
