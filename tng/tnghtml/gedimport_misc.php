...
function adjustID( $ID, $offset, $type ) {
	...
	if( $offset || $trim_it ) {
		...
		$thistrim = (!empty($prefix) && $trimsize[$prefix]) ? $trimsize[$prefix] : strlen( $numericpart );
...
function getMoreInfo( $persfamID, $prevlevel, $prevtag, $prevtype ) {
	...
	elseif($prevtag == "EVEN")
		...
		list($factcont, $lineinfo) = getContinued();
		$moreinfo['FACT'] .= $factcont;
	}
		...
		if( $lineinfo['level'] == $prevlevel ) {
			...
			switch( $tag ) {
				...
				case "ADR1": case "ADR2": case "CITY": case "STAE": case "POST": case "CTRY": case "WWW": case "PHON": case "EMAIL":
					if(!isset($address))
						$address = initAddress();
					list($addrcont, $lineinfo) = getContinued();
					$address[$tag] = addslashes( $lineinfo['rest'] ) . $addrcont;
					break;
				case "PLAC":
				case "TEMP":
				...
				case "_TEXT":
					...
					if( !empty($matches[1]) ) {
						...
						$moreinfo['NOTES'][$notecnt]['NOTE'] .= $notecont;
					}
...
function handleCustomEvent($id,$prefix,$tag) {
	...
	if( $tag == "EVEN" ) {
		...
		list($factcont, $lineinfo) = getContinued();
		$fact = addslashes($oldlineinforest . $factcont);
		$needfact = 1;
	...
	if( $flag ) {
		while( $lineinfo['level'] >= $prevlevel ) {
			if( $lineinfo['level'] == $prevlevel ) {
				...
				switch( $tag ) {
					case "ADR1": case "ADR2": case "CITY": case "STAE": case "POST": case "CTRY": case "WWW": case "PHON": case "EMAIL":
						$rest = addslashes($lineinfo['rest']);
						list($addrcont, $lineinfo) = getContinued();
						$address[$tag] = $rest . $addrcont;
...
function getContinued() {
	...
	return array($continued, $lineinfo);
}
...
function getPlaceRecord($place, $prevlevel) {
	...
	if($place) {
		...
		$changedate = !empty($changedate) ? date( "Y-m-d H:i:s", strtotime( $changedate ) ) : "";
...
function getCropInfo($prevlevel) {
	global $lineinfo;
...
function incrCounter( $prefix ) {
	...
	$newwidth = $fstat['size'] ? ceil(600 * $offset / $fstat['size']) : 0;
    if($old) {
		if($counter % 10 == 0 ) {
	...
	if( $saveimport ) {
		$query = "UPDATE $saveimport_table SET icount = {$savestate['icount']}, fcount = {$savestate['fcount']}, scount = {$savestate['scount']}, mcount = {$savestate['mcount']}, ncount = {$savestate['ncount']}, mcount = {$savestate['mcount']}, fileoffset = $offset, barwidth = $newwidth WHERE gedcom = \"$tree\"";
		$result = @tng_query( $query ) or die ($admtext['cannotexecutequery'] . ": $query");
	}
}
...
function getMoreMMInfo( $prevlevel, $mmcount ) {
	...
	while( $lineinfo['level'] >= $prevlevel ) {
		...
		switch( $tag ) {
			case "FILE":
			case "_FILE":
				list($lineinfo, $moreinfo) = handleMediaFile($lineinfo, $moreinfo);
				break;
			case "TITL":
				$moreinfo[$tag] = addslashes( $lineinfo['rest'] );
				list($titlecont, $lineinfo) = getContinued();
				$moreinfo[$tag] .= $titlecont;
				break;
			case "FORM":
				$slashpos = strpos($lineinfo['rest'], "/");
				$rawform = $slashpos ? substr($lineinfo['rest'], $slashpos + 1) : $lineinfo['rest'];
				$moreinfo['FORM'] = addslashes( strtoupper( $rawform ) );
				$lineinfo = getLine();
				break;
			case "SNOTE":
			case "NOTE":
			...
			case "PLAC":
				...
				break;
			case "_PRIM":
				if($origlevel == 1 && trim($lineinfo['rest']) == "Y")
					$moreinfo['defphoto'] = 1;
				$lineinfo = getLine();
				break;
			case "_PRIV":
				...
				break;
			case "CROP":
				$moreinfo['CROP'] = getCropInfo($lineinfo['level']);
				break;
			default:
				$lineinfo = getLine();
				break;
...
function processMedia( $mmcount, $mminfo, $persfamID, $eventID ) {
	...
	for( $mmctr = 1; $mmctr <= $mmcount; $mmctr++ ) {
		...
		$query = "INSERT IGNORE INTO $media_table (gedcom, mediatypeID, mediakey, path, description, notes, form, usecollfolder, datetaken, placetaken, private, changedate) 
            VALUES(\"$tree\", \"{$mm['mediatypeID']}\", \"{$mm['OBJE']}\", \"{$mm['FILE']}\", \"{$mm['TITL']}\", \"{$mm['NOTE']}\", \"{$mm['FORM']}\", \"1\", \"{$mm['datetaken']}\", \"{$mm['placetaken']}\", \"{$mm['private']}\", \"{$mm['CHAN']}\")";
		$result = @tng_query( $query ) or die ($admtext['cannotexecutequery'] . ": $query");
		...
		$query = "INSERT IGNORE INTO $medialinks_table (personID, mediaID, linktype, altdescription, altnotes, ordernum, dontshow, gedcom, eventID, defphoto, ctop, cleft, cwidth, cheight)
			VALUES(\"$persfamID\", \"$mediaID\", \"{$mm['linktype']}\", \"{$mm['TITL']}\", \"{$mm['NOTE']}\", \"$orderctr\", \"0\", \"$tree\", \"$eventID\", \"{$mm['defphoto']}\", \"{$mm['CROP']['TOP']}\", \"{$mm['CROP']['LEFT']}\", \"{$mm['CROP']['WIDTH']}\", \"{$mm['CROP']['HEIGHT']}\")";
		$result = @tng_query( $query ) or die ($admtext['cannotexecutequery'] . ": $query");
		$psuccess = tng_affected_rows();
		if( !$psuccess && $savestate['del'] != "no" ) {
			$defphotostr = $mm['defphoto'] ? ", defphoto = \"1\"" : "";
			$cropstr = "";
			if(!empty($mm['CROP']['TOP'])) $cropstr .= ", ctop=\"{$mm['CROP']['TOP']}\"";
			if(!empty($mm['CROP']['LEFT'])) $cropstr .= ", cleft=\"{$mm['CROP']['LEFT']}\"";
			if(!empty($mm['CROP']['WIDTH'])) $cropstr .= ", cwidth=\"{$mm['CROP']['WIDTH']}\"";
			if(!empty($mm['CROP']['HEIGHT'])) $cropstr .= ", cheight=\"{$mm['CROP']['HEIGHT']}\"";
			$query = "UPDATE $medialinks_table SET altdescription=\"{$mm['TITL']}\", altnotes=\"{$mm['NOTE']}\"$defphotostr$cropstr 
				WHERE gedcom = \"$tree\" AND personID = \"$persfamID\" AND mediaID = \"$mediaID\" AND eventID=\"$eventID\"";
			$result = @tng_query( $query ) or die ($admtext['cannotexecutequery'] . ": $query");
		}
		if($mm['defphoto']) {
...
function initMultimedia() {
	...
	$mminfo['private'] = 0;
	$mminfo['placetaken'] = "";
	$mminfo['CROP'] = array('TOP' => '', 'LEFT' => '', 'WIDTH' => '', 'HEIGHT' => '');
...
function getMultimediaRecord( $objectID, $prevlevel ) {
	...
	while( $lineinfo['tag'] && $lineinfo['level'] >= $prevlevel ) {
		if( $lineinfo['level'] == $prevlevel ) {
			...
			switch( $tag ) {
				...
				case "FILE":
				case "_FILE":
					list($lineinfo, $mminfo) = handleMediaFile($lineinfo, $mminfo);
					break;
				case "TITL":
				...
				case "FORM":
					$slashpos = strpos($lineinfo['rest'], "/");
					$rawform = $slashpos ? substr($lineinfo['rest'], $slashpos + 1) : $lineinfo['rest'];
					$mminfo['FORM'] = addslashes( strtoupper( $rawform ) );
					$lineinfo = getLine();
					break;
				case "SNOTE":
				case "NOTE":
	...
	if( !$continued ) {
		...
		if( $savestate['del'] != "no" ) {
			...
			$query = "INSERT IGNORE INTO $media_table (gedcom, mediakey, path, thumbpath, description, notes, form, mediatypeID, usecollfolder, datetaken, placetaken, private, changedate) 
                VALUES(\"$tree\", \"{$mminfo['ID']}\", \"{$mminfo['FILE']}\", \"$thumbpath\", \"{$mminfo['TITL']}\", \"{$mminfo['NOTE']}\", \"{$mminfo['FORM']}\", \"{$mminfo['mediatypeID']}\", \"{$mminfo['ucf']}\", \"{$mminfo['datetaken']}\", \"{$mminfo['placetaken']}\", \"{$mminfo['private']}\", \"$inschangedt\")";
			$result = @tng_query( $query ) or die ($admtext['cannotexecutequery'] . ": $query");
...
function handleMediaFile($lineinfo, $moreinfo) {
	$filelevel = $lineinfo['level'];
	$moreinfo['FILE'] = $lineinfo['rest'];
	list($filecont, $lineinfo) = getContinued();
	$moreinfo['FILE'] .= $filecont;
	if( !$moreinfo['mediatypeID'] ) {
		$lastperiod = strrpos($moreinfo['FILE'],".");
		if($lastperiod) {
			$moreinfo['FORM'] = strtoupper(substr($moreinfo['FILE'],$lastperiod + 1));
		}
	...
	while( $lineinfo['level'] > $filelevel ) {
		...
		elseif($lineinfo['tag'] == "TEXT" || $lineinfo['tag'] == "_TEXT") {
			$moreinfo['NOTE'] = addslashes( $lineinfo['rest'] );
			list($notecont, $lineinfo) = getContinued();
			$moreinfo['NOTE'] .= $notecont;
		}
		elseif($lineinfo['tag'] == "DATE" || $lineinfo['tag'] == "_DATE") {
			$moreinfo['datetaken'] = addslashes( trim($lineinfo['rest']) );
			$lineinfo = getLine();
		}
	}
	return array($lineinfo, $moreinfo);
}
...
function getNoteRecord( $noteID, $prevlevel, $notelabel ) {
	...
	preg_match( "/^{$notelabel} ?(.*)$/", $lineinfo['rest'], $matches );
	if( $matches[1] )
		$note = addslashes( $matches[1] );
	else
		$note = "";
	$lineinfo = getLine( );
	if( $lineinfo['level'] && ( $lineinfo['tag'] == $notelabel || $lineinfo['tag'] == "CONT" || $lineinfo['tag'] == "CONC" ) ) {
