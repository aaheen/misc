...
$cemnewoffset = 0;
$cemoffsetplus = 0;
if(!isset($max_cemeteries)) $max_cemeteries = 100;
...
while( !$subquery || $cemetery = tng_fetch_assoc( $cemresult ) )
{
	...
	$query = "SELECT DISTINCT $media_table.mediaID, description, notes, path, thumbpath, status, plot, showmap, usecollfolder, form, mediatypeID, abspath, newwindow, latitude, longitude, $media_table.gedcom
	...
	if( $map['key'] ) {
		...
		if($lat && $long) {
	 		$cemeteryplace = "{$cemetery['cemname']}, {$cemetery['city']}, {$cemetery['county']}, {$cemetery['state']}, {$cemetery['country']}";
			...
			$l2mCount++;
			$body .= "<a href=\"{$http}://maps.google.com/maps?f=q&amp;hl={$text['glang']}$mcharsetstr&amp;daddr=$lat,$long($remoteballoontext)&amp;z=$zoom&amp;om=1&amp;iwloc=addr\" target=\"_blank\"><img src=\"{$cms['tngpath']}google_marker.php?image=$pinplacelevel2.png&amp;text=$l2mCount\" alt=\"\" border=\"0\" align=\"left\" style=\"padding-right:5px\" /></a>";
			$map['pins']++;
		}
	}
	...
	while( $hs = tng_fetch_assoc( $hsresult ) ) {
		...
		if($imgsrc) {
			...
			$body .= "<a href=\"$href\"";
			$treestr2 = $tngconfig['mediatrees'] && $hs['gedcom'] ? $hs['gedcom'] . "/" : "";
			if( $gotImageJpeg && isPhoto($hs) && checkMediaFileSize("$rootpath$usefolder/$treestr2{$hs['path']}") )
				$body .=  " class=\"media-preview\" id=\"img-{$hs['mediaID']}-0-~~" . urlencode("$usefolder/$treestr2{$hs['path']}") . "\"";
			$body .= ">$imgsrc</a>\n";
		}
		else {
			$body .= "&nbsp;";
		...
		$body .= "<td valign=\"top\" class=\"databack\"><a href=\"$href\">{$hs['description']}</a><br />{$hs['notes']}&nbsp;</td>\n";
