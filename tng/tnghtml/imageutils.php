...
function get_pdf_dimensions($path, $box="MediaBox") {
    ...
    while (!$stream->eof()) {
    	$line = $stream->fgets();
        if(preg_match("/".$box."\s*\[\s*[0-9]{1,}[.]{0,}[0-9]{0,} [0-9]{1,}[.]{0,}[0-9]{0,} ([0-9]{1,}[.]{0,}[0-9]{0,}) ([0-9]{1,}[.]{0,}[0-9]{0,})\s*\]/", $line, $matches)) {
...
function image_createThumb( $src, $dest, $maxWidth, $maxHeight, $quality ) { 
    if( file_exists( $src ) && isset( $dest ) ) {
        ...
        if ($destRatio > $srcRatio) { 
            $destSize[1] = round($maxHeight, 0); 
            $destSize[0] = round($maxHeight * $srcRatio, 0); 
        } 
        else { 
            $destSize[0] = round($maxWidth, 0); 
            $destSize[1] = round($maxWidth / $srcRatio, 0); 
        }
        ...
        if($isPDF) {
			...
			if( $gd2 && function_exists( 'imageCreateTrueColor' ) ) {
				...
				if( strtolower($destInfo['extension']) == "gif" || strtolower($destInfo['extension']) == "png" ) {
	        ...
	        switch( $srcSize[2] ) { 
	            case 1: case 2: 
	            	if($srcSize[2] == 2) {
						...
						//dimensions
						$destImage = tngImageRotate($src, $destImage);
					}
...
function tngImageRotate($src, $destImage) {
