...
function editMedia2EntityLink(linkID) {
	tnglitbox = new LITBox('admin_editmedialink.php?linkID='+linkID+'&type='+type,{width:600,height:450});
	return false;
}
	...
	jQuery.ajax({
		...
		success: function(req){
			...
			if(form.type.value != "album") {
				jQuery('#sort_'+linkID+' [href]').attr('href', function(i,a) {
					return a.replace( /(event1=)[0-9]+/ig, '$1'+eventID.options[eventID.selectedIndex].value );
				});
			...
			if(form.type.value != "album") {
				jQuery('#sort_'+linkID).effect('highlight',{},1400);
		...
		jQuery.ajax({
			...
			success: function(req){
				if(req == "1") {
					...
					var actionbuttons = '<a href="#" title="' + edit_text + '" onclick="return editMedia2EntityLink(' + linkID + ');"><img src="img/pen.png" alt="' + edit_text + '" ' + 'class="newsmallericon"/></a>';
					actionbuttons += '<a href="#" title="' + remove_text + '" onclick="return deleteMedia2EntityLink(' + linkID + ');"><img src="img/times.png" alt="' + remove_text + '" ' +'class="newsmallericon"/></a>';
