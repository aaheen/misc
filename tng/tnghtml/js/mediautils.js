...
function assignDefaults(form) {
	...
	var noeventmedia = form.noeventmedia.checked ? 1 : "";
	var tree = form.tree.options[form.tree.selectedIndex].value;
	jQuery('#defspin').show();
	var params = {overwritedefs:overwrite,noeventmedia:noeventmedia,tree:tree};
	jQuery.ajax({
...
function editMediatype(field){
	...
	tnglitbox = new LITBox('admin_editcollection.php?field='+fieldname+'&mediatypeID='+mediatypeID+'&selidx='+field.selectedIndex,{width:600,height:600});
}
...
function updateCollection(form) {
	if(form.display.value == "")
		...
		jQuery.ajax({
			...
			dataType: 'html',
			success: function(req){
				var field = document.form1[form.field.value];
