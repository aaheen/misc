...
<tr class="databack">
	<td class="tngshadow">
		...
		<p>Check this box prior to clicking the Search button to restrict the results to media that are marked as Private.</p>
...
<tr class="databack">
	<td class="tngshadow">
		...
		an absolute web address (for example, <em>https://www.thatsite.com/image.jpg</em>) in the field labeled "Media URL", and
		...
		program or online file manager to create the directory and give it proper rights (755 should work, but 777 is required on some sites). </p>
		...
		<span class="optionhead">Cemetery</span>
		<p>To associate this media item with a cemetery, select the cemetery from the dropdown box. Typically this is used with headstones to indicate where the headstone is located. You must first add the cemetery
...
<tr class="databack">
	<td class="tngshadow">
		...
		also change the <strong>Default Photo</strong> or <strong>Show</strong> status for each link by checking the appropriate box, and you can add cropping just
		for this link. See below for more information on these attributes.</p>
		...
		<span class="optionhead">Cropping</span>
		<p>To crop the image just for this link, enter the number of pixels from the top and left of the image where the upper left corner of the visible area should be,
			then also enter the height and width of the visible area. You might need to use a separate photo editor to get the right numbers (or just experiment).
		</p>
		...
		different portions of the image to individuals, families, sources, repositories or places in your database, or to external URLs. Or just 
        enter a Title to make an informational tag that doesn't link anywhere.</p>

		<p><strong>Note</strong>: The media item must be a valid JPG, GIF or PNG image to use this feature.</p>

		<p>For each region you want to link, first select the tree and entity type (individual, family, etc), OR enter the title and external URL for another web page. 
		Then define the region by drawing a rectangle on the image with your
		...
		select your image coordinates. After the coordinates have been selected, a popup box will be displayed to allow you to find or enter the entity's ID. Enter
		all or part of the name or ID to locate possible matches, then choose the correct item from the candidates displayed. The
...
<tr class="databack">
	<td class="tngshadow">
		...
		unchecked allows previously set defaults to remain, which may be especially useful after uploading a GEDCOM file that already has default photos set.</p>
	</td>
</tr>
<tr class="databack">
	<td class="tngshadow">
