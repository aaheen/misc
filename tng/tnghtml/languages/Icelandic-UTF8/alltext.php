<?php
$alltextloaded = 1;

$dates['JAN'] = "jan.";
$dates['JANUARY'] = "janúar";
$dates['FEB'] = "feb.";
$dates['FEBRUARY'] = "febrúar";
$dates['MAR'] = "mar.";
$dates['MARCH'] = "mars";
$dates['APR'] = "apr.";
$dates['APRIL'] = "apríl";
$dates['MAY'] = "maí";
$dates['JUN'] = "jún.";
$dates['JUNE'] = "júní";
$dates['JUL'] = "júl.";
$dates['JULY'] = "júlí";
$dates['AUG'] = "ágú.";
$dates['AUGUST'] = "ágúst";
$dates['SEP'] = "sep.";
$dates['SEPTEMBER'] = "september";
$dates['OCT'] = "okt.";
$dates['OCTOBER'] = "október";
$dates['NOV'] = "nóv.";
$dates['NOVEMBER'] = "nóvember";
$dates['DEC'] = "des.";
$dates['DECEMBER'] = "desember";
$dates['ABT'] = "um";
$dates['ABOUT'] = "um";
$dates['BEF'] = "fyrir";
$dates['BEFORE'] = "fyrir";
$dates['AFT'] = "eftir";
$dates['AFTER'] = "eftir";
$dates['BET'] = "Milli";
$dates['BETWEEN'] = "milli";
$dates['TEXT_AND'] = "Og";
$dates['FROM'] = "Frá";
$dates['TO'] = "til";
$dates['Y'] = "Já, dagsetning ókunn";
$dates['CAL'] = "Cal";
$dates['EST'] = "Ca.";

//global messages
$text['cannotexecutequery'] = "Ekki hægt að verða við beiðni";
$text['to'] = "til";
$text['of'] = "af";
$text['text_next'] = "Næsta";
$text['text_prev'] = "Fyrri";
$text['clickdisplay'] = "Smelltu til að sýna";
$text['clickhide'] = "Smelltu til að fela";
$text['forgot1'] = "<strong>Ertu búinn að gleyma notendanafninu eða lykilorðinu?</strong><br />Sláðu inn netfangið þitt hér fyrir neðan og við sendum þér notendanafnið.";
$text['forgot2'] = "Sláðu inn netfangið hér fyrir ofan og notendanafnið fyrir neðan til að fá nýtt lykilorð (þér verður sent tímabundið lykilorð).";
$text['newpass'] = "Nýja bráðarbirgða lykilorðið þitt";
$text['usersent'] = "Notendanafnð hefur verið sent á netfangið þitt.";
$text['pwdsent'] = "Nýja bráðarbirgða lykilorðið hefur verið sent á netfangið þitt.";
$text['loginnotsent2'] = "Þetta netfang er ekki til á skrá hjá okkur, engar upplýsingar hafa verið sendar.";
$text['loginnotsent3'] = "Þetta netfang og/eða notendanafn er ekki til á skrá hjá okkur, engar upplýsingar hafa verið sendar.";
$text['logininfo'] = "Notendanafnið þitt";
$text['collapseall'] = "Fella allt saman";
$text['expandall'] = " Þenja allt út";

//media types
$text['photos'] = "Ljósmyndir";
$text['documents'] = "Skjöl";
$text['headstones'] = "Legsteinar";
$text['histories'] = "Sögur";
$text['recordings'] = "Upptökur";
$text['videos'] = "Myndbönd";

//For Google maps use - admin and public pages
$admtext['placelevel'] = "Gerð staðar";
$admtext['level1'] = "Heimilisfang";
$admtext['level2'] = "Staðsetning";
$admtext['level3'] = "Bær/Borg";
$admtext['level4'] = "Hreppur";
$admtext['level5'] = "Sýsla";
$admtext['level6'] = "Land";
$admtext['level0'] = "Ekki stillt";

$text['male'] = "Maður";
$text['female'] = "Kona";
$text['closewindow'] = "Loka þessum glugga";
$text['loading'] = "Hleð inn síðu...";

$text['cancel'] = "Hætta við";
$text['none'] = "enginn";
$text['mainton'] = "Viðhald í gangi";

$text['living'] = "Á lífi";
$admtext['text_private'] = "Falin(n)";
$admtext['confunlink'] = "Ertu viss um að þú viljir aftengja þennan einstakling sem maka í þessari fjölskyldu?";
$admtext['confunlinkc'] = "Ertu viss um að þú viljir aftengja þennan einstakling sem barn í fjölskyldunni?";
$admtext['confremchild'] = "Ertu viss um að þú viljir fjarlægja þetta barn frá fjölskyldunni? Einstklingurinn verður ekki eytt úr gagnagrunninum.";
$admtext['enterfamilyid'] = "Sláðu inn fjölskyldu ID.";
$admtext['yes'] = "Já";
$admtext['no'] = "Nei";
$admtext['BIRT'] = "Fæðing";
$admtext['DEAT'] = "Andlát";
$admtext['CHR'] = "Skírn";
$admtext['BURI'] = "Jarðsetning";
$admtext['BAPL'] = "Skírn (LDS)";
$admtext['ENDL'] = "Endowment (LDS)";
$admtext['NICK'] = "Nickname";
$admtext['TITL'] = "Title";
$admtext['NSFX'] = "Suffix";
$admtext['NAME'] = "Fornafn";
$admtext['SLGC'] = "Sealed to Parents (LDS)";
$admtext['MARR'] = "Gifting";
$admtext['SLGS'] = "Sealed to Spouse (LDS)";
$admtext['hello'] = "Halló";
$admtext['activated'] = "Notendareikningur þinn hefur verið búinn til.";
$admtext['infois'] = "Skráningarupplýsingarnar þínar eru";
$admtext['subjectline'] = "Ættfræði - Notendareikningur þinn hefur verið búinn til";
$admtext['adopted'] = "Ættleiðing";
$admtext['birth'] = "fæðing";
$admtext['foster'] = "Fóstur";
$admtext['sealing'] = "sealing";
$admtext['putative'] = "putative";

$text['editprofile'] = "Breyta notendaupplýsingum";
$text['letter'] = "Bréf"; //as in page size
$text['legal'] = "Löglegt"; //as in page size
$text['sunday'] = "Sunnudagur";  //first day of week on calendar
$text['monday'] = "Mánudagur";
$text['tuesday'] = "Þriðjudagur";
$text['wednesday'] = "Miðvikudagur";
$text['thursday'] = "Fimmtudagur";
$text['friday'] = "Föstudagur";
$text['saturday'] = "Laugardagur";
$text['contains'] = "inniheldur";
$text['startswith'] = "byrjar á";

$text['top'] = "Efst";
$text['startingind'] = "Fyrsti einstaklingur";
$text['enteremail'] = "Vinsamlegast sláðu inn netfang.";
$text['page'] = "Síða";
$text['go'] = "Áfram";
$text['years'] = "ára";
$admtext['CONL'] = "Confirmation (LDS)";
$admtext['INIT'] = "Initiatory (LDS)";
$admtext['step'] = "Stjúpbarn";
$text['switchm'] = "Switch to mobile site";
$text['switchs'] = "Switch to standard site";
$text['dna_tests'] = "DNA próf";
$admtext['preflang'] = "Valið tungumál";
$text['query'] = "Fyrirspurn";
$text['fanchart'] = "Ættarhringur";

$admtext['menu'] = "Valmynd";
$admtext['dna_groups'] = "DNA hópar";
$admtext['allbranches'] = "Allar greinar";
$admtext['active'] = "Virk";
$admtext['select'] = "Velja";
$admtext['selectall'] = "Velja allt";
$admtext['clearall'] = "Hreinsa allt";
$admtext['medialinks'] = "Tengingar við margmiðlun";

$text['testgroup'] = "Prófhópur";
$text['test_info_display'] = "Test Information To Display";
$text['confirmed'] = "Staðfest";
$text['predicted'] = "Spáð";
$admtext['ancestral_surnames'] = "Eftirnöfn forfeðra";
$admtext['admin_notes'] = "Athugasemdir stjórnanda";
$admtext['mda'] = "Fjarskyldasti forfaðirinn";
$admtext['mrca'] = "Most recent common ancestor";
$admtext['marker_values'] = "Marker values";
$admtext['ref_seq'] = "Reference sequence";
$admtext['rsrs'] = "RSRS (Reconstructed Sapiens Reference Sequence)";
$admtext['rcrs'] = "rCRS (revised Cambridge Reference Sequence)";
$admtext['hvr_values'] = "HVR Differences";
$admtext['hvr1_values'] = "HVR1 Differences";
$admtext['hvr2_values'] = "HVR2 Differences";
$admtext['coding_reg'] = "Coding region differences";
$text['keep_name_private'] = "Halda nöfnum földum";
$text['test_results'] = "Niðurstöður";
$admtext['dna_tests'] = "DNA próf";
$admtext['vendor'] = "Vendor";
$admtext['markers'] = "Number of STR markers tested";
$admtext['haplo'] = "haplohópur";
$admtext['termsnp'] = "Terminal SNP";
$admtext['signsnp'] = "Significant SNPs";
$admtext['atdna_test'] = "atDNA (Autosomal)";
$admtext['ydna_test'] = "Y-DNA";
$admtext['mtdna_test'] = "mtDNA (hvatbera)";
$admtext['xdna_test'] = "X-DNA";
$admtext['xtra_mut'] = "Extra mutations";
$text['test_number'] = "Heiti/númer prófs";
$admtext['match_date'] = "Match Date";
$admtext['gedmatchID'] = "GEDmatch ID";
$admtext['ydna_haplogroup'] = "Y-DNA Haplogroup";
$admtext['mtdna_haplogroup'] = "mtDNA Haplogroup";
$admtext['relationship_section'] = "Relationship Section";
$admtext['relationship_range'] = "Relationship Range";
$admtext['suggested_relationship'] = "Suggested";
$admtext['actual_relationship'] = "Actual Relationship";
$admtext['related_side'] = "Related Branch";
$admtext['shared_dna'] = "Deilt DNA";
$admtext['shared centimorgans'] = "Total shared cMs";
$admtext['shared_segments'] = "Shared segments";
$admtext['largest_segment'] = "Largest Segment";
$admtext['chromosome'] = "Litninganúmer";
$admtext['segment_start'] = "Segment Start";
$admtext['segment_end'] = "Segment End";
$admtext['centiMorgans'] = "centiMorgans";
$admtext['matchingSNPs'] = "Matching SNPs";
$admtext['xmatch'] = "X-Match";
$text['keep_test_private'] = "Private Test";
$text['shared_dna'] = "Shared DNA Segment";
$text['for_match'] = "for this match";
$admtext['keeptest'] = "Test Privacy";
$admtext['keepname'] = "Name Privacy";
$admtext['allgroups'] = "Allir hópar";
$admtext['alltypes'] = "Allar tegundir";
$admtext['generation_num'] = "Generations For Ancestral Surnames";

$text['birthabbr'] = "f.";
$text['chrabbr'] = "s.";
$text['deathabbr'] = "d.";  //from descend
$text['burialabbr'] = "graf.";  //from descend
$text['marrabbr'] = "G.";

//moved here fron text.php in TNG 13.0
$text['findplaces'] = "Finna alla einstaklinga með viðburði á þessum stað";
$text['firstpage'] = "Fyrsta síða";
$text['lastpage'] = "Síðasta síða";

//changed in 13.0.0
$text['problem'] = "Villa hefur komið upp í TNG forritinu. Hvað skal gera:<p>Ef þú ert nýbúin(n) að setja inn uppfærslu, þá gæti verið að þú hafir hoppað yfir hlutann með uppsetningarleiðbeiningum. Farðu aftur í readme skýrslu uppfærslunnar og lestu leiðbeiningarnar yfir aftur. Skoðaðu hlutann um uppbyggingu gagnagrunnins sérstaklega vel.</p><p>Ef þú varst að setja TNG upp í fyrsta sinn, gæti verið að þig vanti að búa til töflurnar í gagnagrunninum. Farðu aftur á readme.html síðuna til að finna það skref í leiðbeiningunum.</p><p>Ef þetta er heimasíðan þín, geturðu haft samband við TNG aðstoðina (þar sem þú keyptir forritið). Vinsamlegast afritið fyrirspurnina (query) hér fyrir neðan og settu það inn í tölvupósinn.</p>";
//added in 13.0.0
$admtext['nobranch'] = "Engin grein";

//added in 14.0
$text['oneyear'] = "ár";
$text['pwdcodelink'] = "Smelltu á þennan tengil til að fá sent tímabundið lykilorð.";
$text['pwdcodesent'] = "Þér hefur verið sendur hlekkur til að endursetja lykilorðið þitt.";
$text['moved'] = "Flutt";
$text['citation'] = "Heimild";
$text['other'] = "Aðrar upplýsingar";
$text['death'] = "Andlát";
$text['burial'] = "Greftrun";
$text['cremation'] = "Líkbrennsla";
$text['marriage'] = "Hjónaband";
$text['divorce'] = "Skilnaður";
$text['bylength'] = "Skref";
$text['bymarriages'] = "Hjónabönd";
$text['none'] = "enginn";

//moved here in 14.0
$text['family'] = "Fjölskylda";
$text['source'] = "Heimildir";
$text['repository'] = "Greftrunarstaður";
$text['place'] = "Staður";
$admtext['BAPM'] = "Skírn";

//added in 15.0
$text['back'] = "Back";
$text['all'] = "Allt";
$admtext['longerpwd'] = "Passwords must be between xxx and 128 characters long.";

//moved here in 15.0
$admtext['choosedef'] = "Choose<br/>default<br/>photo"; //has to fit in a small box
$admtext['norights'] = "Þú hefur ekki leyfi til að skoða þessa síðu.  Ef þú hefur notendanafn og lykilorð, skráðu þig þá inn fyrir neðan.";
$admtext['addmedia'] = "Bæta við margmiðlun";
$text['firstnames'] = "Fornöfn";
$text['searchnames'] = "Leita í nöfnum";
$text['searchfams'] = "Leita í fjölskyldum";
$text['searchsitemenu'] = "Leita í öllum gögnum";
$text['dates'] = "Dagsetningar";
$text['calendar'] = "Dagatal";
$text['bookmarks'] = "Bókamerki";
$text['mnushowlog'] = "Loggar";
$text['contactus'] = "Hafðu samband";

$text['glang'] = "is";

// loading English or English-UTF8 is required to display custom text for mods that do not provide all translations
$english = $_SESSION['session_charset'] == "UTF-8" ? "English-UTF8" : "English";
@include($rootpath . $endrootpath . "languages/$english/cust_text.php");
@include(dirname(__FILE__) . "/cust_text.php");
?>