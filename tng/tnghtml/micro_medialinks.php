	...
	if( $result2 ) {
		...
		while( $plink = tng_fetch_assoc( $result2 ) )
		{
			...
			elseif( $plink['sourceID'] != NULL ) {
				...
				$id = " ($entityID)";
				$name = truncateIt($plink['sourcetitle'],100);
				$linktype = "S";
			}
			...
			$linkrows .= "<a href=\"#\" title=\"{$admtext['edit']}\" onclick=\"return editMedia2EntityLink({$plink['mlinkID']});\" title=\"{$admtext['edit']}\" class=\"newsmallericon\"><img src=\"img/pen.png\" /></a>";
			$linkrows .= "<a href=\"#\" title=\"{$admtext['removelink']}\" onclick=\"return deleteMedia2EntityLink({$plink['mlinkID']});\" title=\"{$admtext['removelink']}\" class=\"newsmallericon\"><img src=\"img/times.png\" /></a>";
			$linkrows .= "</td>\n";
			$linkrows .= "<td class=\"lightback normal\">" . $admtext[$type] . "</td><td class=\"lightback normal\" id=\"sort_{$plink['mlinkID']}\">$name$id";
			if($linktype != "C")
			...
			if($linktype != "C")
				$linkrows .= "<input type=\"checkbox\" name=\"defc{$plink['mlinkID']}\" id=\"defc{$plink['mlinkID']}\" onclick=\"toggleDefault(this,'{$plink['gedcom']}','" . addslashes($entityID) . "');\" value=\"1\"$dchecked\"/>";
			$linkrows .= "</td>\n";
...
?>
	...
	<table cellspacing="2">
		...
		<tr>
			<td>
				<select name="tree1" class="bigselect" id="microtree">
<?php
				for( $j = 1; $j <= $treenum; $j++ ) {
...
?>
			...
			<td class="normal"><input type="button" class="btn" value="<?php echo $admtext['add']; ?>" onclick="return addMedia2EntityLink(findform);"> &nbsp;<?php echo $admtext['text_or']; ?>&nbsp;</td>
			<td><a href="#" onclick="return findItem(findform.linktype1.options[findform.linktype1.selectedIndex].value,'newlink',null,findform.tree1.options[findform.tree1.selectedIndex].value,'<?php echo $assignedbranch; ?>','m_<?php echo $mediaID; ?>');" class="newsmallicon" title="<?php echo $admtext['find']; ?>">
