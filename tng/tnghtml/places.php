...
if( $result ) {
	...
	while( $place = tng_fetch_assoc( $result ) ) {
		...
		if( $place2 != "" ) {
			...
			$name = $place['placecount'] > 1 || !$specificcount ? "<a href=\"$places_oneletter_url" . "offset=$offset&amp;{$treestr}psearch=$place2\">" . str_replace(array("<",">"), array("&lt;","&gt;"), $place['myplace']) . "</a> <span class=\"lighttext\">($tally_fmt)</span>" : $place['myplace'];
			if(($count - 1) % $collen == 0)
				$col++;
			...
			$linkstr2col[$col] .= "<tr><td class=\"snline\" align=\"right\">$count.</td><td>$name$searchlink</td>$chartstr</tr>\n";
			$count++;
		}
	}
	tng_free_result($result);
...
?>
<?php
echo "{$text['placescont']}: <input type=\"text\" name=\"psearch\" id=\"psearch\" onkeyup=\"if(!jQuery(this).val()) {jQuery('#pgo').attr('disabled','disabled');} else {jQuery('#pgo').removeAttr('disabled');}\"/>\n";
if($tree && !$tngconfig['places1tree'])
    echo "<input type=\"hidden\" name=\"tree\" value=\"$tree\" />\n";
echo "<input type=\"hidden\" name=\"stretch\" value=\"1\" />\n";
echo "<input type=\"submit\" name=\"pgo\" id=\"pgo\" onsubmit=\"if(!jQuery('#psearch').val()){return false;}\" disabled=\"disabled\" value=\"{$text['go']}\" />\n";
?>
</form>
