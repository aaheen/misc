...
$varlist = array('bpqualify','branch','brpqualify','bryqualify','byqualify','cpqualify','cyqualify','dpqualify','dyqualify','ecount','fnqualify','idqualify','lnqualify','gequalify','myaltbirthplace','myaltbirthyear','mybirthplace','mybirthyear','myburialplace','myburialyear','cremated','mydeathplace','mydeathyear','myfirstname','mygender','mylastname','mynickname','mypersonid','myprefix','mysplname','mysuffix','mytitle','nnqualify','pfqualify','sfqualify','showdeath','showspouse','spqualify','tngprint','tqualify','urlstring');
...
$_SESSION['tng_search_cremated'] = $cremated;
...
$nameorderstr = $tngconfig['searchorder'] ? "p.firstname, p.lastname" : "p.lastname, p.firstname";
$nameorderdescstr = $tngconfig['searchorder'] ? "p.firstname DESC, p.lastname DESC" : "p.lastname DESC, p.firstname DESC";

if($order == "birth") {
	$orderstr = "IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr), $nameorderstr";
	...
	if($order == "birthup")
		$orderstr = "IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr) DESC, $nameorderstr";
}

if($order == "death") {
	$orderstr = "IF(p.deathdatetr, p.deathdatetr, p.burialdatetr), $nameorderstr, IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr)";
	...
	if($order == "deathup")
		$orderstr = "IF(p.deathdatetr, p.deathdatetr, p.burialdatetr) DESC, $nameorderstr, IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr)";
}

$nametitle = $sitever == "mobile" ? $text['name'] : $text['lastfirst'];
if($order == "name") {
	$orderstr = "$nameorderstr, IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr)";
	...
	if($order == "nameup")
		$orderstr = "$nameorderdescstr, IF(p.birthdatetr, p.birthdatetr, p.altbirthdatetr)";
}
...
$dontdo = array(); //array("ADDR","BIRT","CHR","DEAT","BURI","NICK","TITL","NSFX");
...
if( $tree ) {
	...
	$querystring .= $text['tree'] . " {$text['equals']} ";
	if(isset($treerow['treename']))
		$querystring .= $treerow['treename'];
...
if(($mysplname && $mygender ) || $spqualify == "exists" || $spqualify == "dnexist") {
	...
	$query = "SELECT p.ID, spouse.personID as spersonID, p.personID, p.lastname, p.lnprefix, p.firstname, p.nickname, p.living, p.private,
		p.branch, p.suffix, p.prefix, p.nameorder, p.title, p.birthplace, p.birthdate, p.birthdatetr, p.deathplace, p.deathdate, p.deathdatetr,
		p.altbirthdate, p.altbirthdatetr, p.altbirthplace, p.burialdate, p.burialplace, p.gedcom, treename
...
echo "<p>{$text['matches']} $offsetplus {$text['to']} $numrowsplus {$text['of']} " . number_format($totrows) . " $querystring</p>";
...
if ($sitever != "standard") {
	if ($tabletype == "toggle") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"columntoggle\"" . $headerr;
	} elseif ($tabletype == "stack") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"stack\"" . $headerr;
	} elseif ($tabletype == "swipe") {
		$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"swipe\"" . $headerr;
	} else{
...
<?php
		if( $mydeathyear || $mydeathplace || $myburialyear || $myburialplace || $showdeath || $cremated ) { 
?>
	...
	if($numtrees > 1 || $numbranches) {
		...
		if($row['branch']) {
			...
			foreach($branches as $branch) {
				...
				$branchname = isset($branchnames[$key]) ? $branchnames[$key] : "";
				if(!$branchname) {
