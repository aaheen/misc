...
if($sitever != "standard") {
	...
	$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" style=\"maxwidth: 350px; width:100%\" class=\"tablesaw thfixed whiteback\" data-tablesaw-mode=\"$tabletype\"{$headerr}>\n";
} else {
	$header = "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" class=\"thfixed whiteback\" style=\"width:100%\">";
}
echo "<div class=\"tngstats\">\n<div class=\"tngstats2 statstwothirds\">\n<div class=\"statshalf\">\n";
echo $header;
?>
	<thead>
		<tr>
		<th data-tablesaw-priority="persist" class="fieldnameback fieldname">&nbsp;<?php echo $text['description']; ?>&nbsp;</th>
		<th data-tablesaw-priority="1" class="fieldnameback align-right fieldname nw" width="40%" align="center">&nbsp;<?php echo $text['quantity']; ?>&nbsp;</th>
		</tr>
	</thead>
<?php
...
foreach( $mediatypes as $mediatype ) {
	if($mediatype['statistics'] && !$mediatype['disabled']) {
...
$query = "SELECT count(citationid) as scitcount FROM $citations_table $wherestr";
$result =  tng_query($query);
$row = tng_fetch_assoc( $result );
$totalsourcecitations = number_format($row['scitcount']);
tng_free_result($result);
...
echo "<tr><td class=\"databack\">{$text['totindividuals']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$totalpeople&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totmales']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$fmt_males ($percentmales%)&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totfemales']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$fmt_females ($percentfemales%)&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totunknown']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$fmt_unknownsex ($percentunknownsex%)&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totliving']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$numliving&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totfamilies']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$totalfamilies&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totuniquesn']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$uniquesurnames&nbsp;</td></tr>\n";
...
foreach( $mediatypes as $mediatype ) {
	if($mediatype['statistics'] && !$mediatype['disabled']) {
		...
		echo "<tr><td class=\"databack\">{$text['total']} $titlestr</td>\n";
		echo "<td class=\"databack\" align=\"right\">" . number_format($totalmedia[$mediatypeID]) . "&nbsp;</td></tr>\n";
...
echo "<tr><td class=\"databack\">{$text['totsources']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$totalsources&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['totalsourcecitations']}</td>\n";
echo "<td class=\"databack\" align=\"right\">$totalsourcecitations&nbsp;</td></tr>\n";

echo "<tr><td class=\"databack\">{$text['avglifespan']}<sup><font size=\"1\">1</font></sup></td>\n";
echo "<td class=\"databack\" align=\"right\"><nobr>$avgyears {$text['years']}&nbsp;</nobr><wbr /><nobr>$avgdays {$text['days']}&nbsp;</nobr></td></tr>\n";

echo "<tr><td class=\"databack\">" . $text['earliestbirth'];
if($firstallowed)
...
echo "&nbsp;</td>\n";
echo "<td class=\"databack\" align=\"right\">" . displayDate( $firstbirthdate ) . "&nbsp;</td></tr>\n";

if( !empty($tngconfig['lastimport']) && !empty($treerow['treename']) && $lastimportdate ) {
	echo "<tr><td class=\"databack\">" . $text['lastimportdate'] . "</td>\n";

	$importtime = strtotime($lastimportdate);
	if(substr($lastimportdate,11,8) != "00:00:00")
		$importtime += ($time_offset * 3600);
	$importdate = date("d M Y H:i:s", $importtime);

	echo "<td class=\"databack\" align=\"right\">" . displayDate( $importdate ) . "&nbsp;</td></tr>\n";
}
?>
</table>
<br />
...
?>
	<thead>
		...
		<th data-tablesaw-priority="1" class="fieldnameback align-right fieldname nw" width="40%" align="center">&nbsp;<?php echo $text['age']; ?>&nbsp;</th>
		</tr>
	</thead>
<?php
...
while ($line = tng_fetch_array($result, 'assoc')) 
{
	...
	echo "<tr><td valign=\"top\" class=\"databack\"><a href=\"$getperson_url" . "personID=$personid&amp;tree=$gedcom\">";
	echo getName($line);
	echo "</a>&nbsp;</td>\n";
	echo "<td valign=\"top\" class=\"databack\" align=\"right\">";
	if( $yearsold )
		echo "<nobr>" . number_format($yearsold) . " " . $text['years'] . "</nobr>";
	if( $daysold )
		echo "<wbr />&nbsp;<nobr>$daysold " . $text['days'] . "</nobr>";
	echo "&nbsp;</td></tr>\n";
}
echo "</table>\n";
...
<?php
echo "<br/>\n";
$width = $sitever == "standard" ? "95%" : "100%";
echo "<table style=\"width:$width\" cellpadding=\"3\" cellspacing=\"1\" border=\"0\" class=\"tfixed whiteback\">\n";
echo "<tr><td  valign=\"top\" class=\"fieldnameback\" style=\"width:10px\"><span class=\"fieldname\"><sup><font size=\"1\">1</font></sup>&nbsp;</span></td>";
echo "<td class=\"databack\">{$text['agedisclaimer']}</td></tr>";
echo "</table>\n";
tng_free_result($result);

if( $tree && !$treerow['secret'] ) {
	echo "<br/>\n";
	echo "<a href=\"$showtree_url" . "tree=$tree\">{$text['treedetail']}</a>\n";
	echo "<br/>\n";
}
?>
