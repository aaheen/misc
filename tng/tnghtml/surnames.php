...
if( $result ) {
	...
	while( $surname = tng_fetch_assoc( $result ) ) {
		...
		$linkstr2col[$col] .= "<tr><td class=\"snline\" align=\"right\">$count.</td><td class=\"nw\"><a href=\"$search_url" . "mylastname=$surname2&amp;lnqualify=equals&amp;mybool=AND$treestr\">{$surname['lowername']}</a> <span class=\"lighttext\">($tally_fmt)</span></td>$chartstr</tr>";
		$count++;
	}
	tng_free_result($result);
}
...
<?php
	if(!empty($linkstr2col[$i])) echo $linkstr2col[$i];
?>
			</table>
		</td>
<?php
...
?>
			<input type="text" name="topnum" value="100" size="4" maxlength="4" /> <?php echo $text['byoccurrence']; ?>  <input type="hidden" name="tree" value="<?php echo $tree; ?>" /><input type="submit" class="btn" value="<?php echo $text['go']; ?>" /></form>
		</td>
	</tr>
