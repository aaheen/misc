...
if(empty($primaryID)) pageNotFound();

include($subroot . "pedconfig.php" );
include("timelineconfig.php" );

if( !isset($pedigree['event']) ) $pedigree['event'] = "over";
$timeline = isset($_SESSION['timeline']) ? $_SESSION['timeline'] : "";
$tng_message = isset($_SESSION['tng_message']) ? $_SESSION['tng_message'] : "";
...
function getEvents($person) {
	...
	foreach( $events as $event ) {
		...
		$eventstr .= "<table border=\"0\" cellspacing=\"0\" cellpadding=\"1\"><tr><td class=\"pboxpopup nw\">&gt; ";
		$eventstr .= "{$event['year']} - {$event['text']} {$event['date']} &nbsp;</td></tr></table></div>\n";
	}
...
foreach( $timeline as $timeentry ) {
	...
	$query = "SELECT firstname, lnprefix, lastname, prefix, suffix, nameorder, title, living, private, branch, gedcom, sex, personID,
...
$flags['css'] = "<link href=\"{$cms['tngpath']}css/timeline.css?v=" . (filemtime($rootpath."css/timeline.css")) . "\" rel=\"stylesheet\" type=\"text/css\" />\n";
...
$innermenu .= "<input type=\"text\" name=\"newwidth\" class=\"minifield\" value=\"$chartwidth\" maxlength=\"4\" size=\"4\" style=\"height:18px\" /> &nbsp;&nbsp; ";
...
if( $tng_message ) {
	echo "<p><strong>$tng_message</strong></span>/p>";
...
echo "<div style=\"line-height:1.1em\"><br/><br/>\n";
if( count( $timeline ) > 1 )
...
foreach( $keeparray as $timeentry ) {
	...
	echo "<table cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"nw\">{$timeentry['name']}</td></tr><tr><td><div class=\"fieldnameback\" style=\"font-size:0;height:10px;padding:0px;width:$spanwidth" . "px;z-index:3\"></div></td></tr></table>\n";
	echo "</div>\n";

	echo "<div id=\"popup$numlines\" class=\"popup\" style=\"background-color:{$pedigree['popupcolor']}; top:" . ($top + 25) . "px; left:" . ($spanleft - 5) . "px;\" onmouseover=\"cancelTimer($numlines)\" onmouseout=\"setTimerHide($numlines)\">\n";
	echo "<table class=\"popuptable\" cellpadding=\"1\" cellspacing=\"0\"><tr><td>\n";
...
<?php
	...
	for( $x = 2; $x < 6; $x++ ) {
		echo $text['addperson'] . ": ";
		if( $numrows > 1 ) {
			echo "<select name=\"nexttree$x\" class=\"bigselect\">\n";
		...
		echo "<input type=\"text\" name=\"nextpersonID$x\" id=\"nextpersonID$x\" size=\"10\" />  <input type=\"button\" name=\"find$x\" id=\"find$x\" class=\"btn\" value=\"{$text['find']}\" onclick=\"findItem('I','nextpersonID$x',null,$treestr);\" /><br/>\n";
		if( $x < 5 )
			$treeresult = tng_query($query);
	}
?>
...
<table cellpadding="3" cellspacing="1" border="0" width="100%" class="thfixed whiteback">
	<thead>
		<tr>
			<th class="fieldnameback" width="20">&nbsp;</th>
			<th class="fieldnameback fieldname" width="50"><strong>&nbsp;<?php echo $text['date']; ?></strong></th>
			<th class="fieldnameback fieldname"><strong>&nbsp;<?php echo $text['event']; ?></strong></th>
		</tr>
	</thead>
<?php
		$counter = 0;
		foreach( $tlevents2 as $key=>$value ) {
			$counter++;
			echo "<tr>\n";
			echo "<td class=\"databack\" valign=\"top\" align=\"right\">$counter&nbsp;</td>";
			echo "<td class=\"databack\" valign=\"top\">$key&nbsp;</td>";
			echo "<td class=\"databack\" valign=\"top\"><ul>$value</ul></td>";
			echo "</tr>\n";
		}
?>
</table><br/>
...
function setTimerHide(slot) {
	...
	timers[slot] = false;
	timers[slot] = setTimeout("hidePopup('" + slot + "')",<?php echo $pedigree['popuptimer']; ?>);
}

function setTimerShow(slot,ev) {
	if(ev == "over")
		timers[slot] = setTimeout("showPopup('" + slot + "')",<?php echo $pedigree['popuptimer']; ?>);
	else
		showPopup(slot);
}
